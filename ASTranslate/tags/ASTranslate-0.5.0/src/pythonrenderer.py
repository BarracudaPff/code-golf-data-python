...
def renderobject(obj):
	if isinstance(obj, list):
		...
		return repr(obj)
def renderCommand(appPath, addressdesc, eventcode, targetRef, directParam, params, resultType, modeFlags, timeout, appdata):
	args = []
	if not _commandscache.has_key((addressdesc.type, addressdesc.data)):
		_commandscache[(addressdesc.type, addressdesc.data)] = dict([(data[1][0], (name, dict([(v, k) for (k, v) in data[1][-1].items()]))) for (name, data) in appdata.referencebyname().items() if data[0] == "c"])
