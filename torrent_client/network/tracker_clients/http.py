...
from torrent_client.network.tracker_clients.base import BaseTrackerClient, TrackerError, parse_compact_peers_list, EventType
...
class HTTPTrackerClient(BaseTrackerClient):
	...
	async def announce(self, server_port: int, event: EventType):
		params = {"info_hash": self._download_info.info_hash, "peer_id": self._our_peer_id, "port": server_port, "uploaded": self._statistics.total_uploaded, "downloaded": self._statistics.total_downloaded, "left": self._download_info.bytes_left, "compact": 1}
		if event != EventType.none:
			params["event"] = event.name
		if self._tracker_id is not None:
			params["trackerid"] = self._tracker_id
		params = {name: urllib.parse.quote(value) if isinstance(value, bytes) else value for name, value in params.items()}
