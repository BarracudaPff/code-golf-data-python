...
class PyMouseEvent(PyMouseEventMeta):
	def __init__(self, display=None):
		...
		self.display = Display(display)
		self.display2 = Display(display)
		self.ctx = self.display2.record_create_context(0, [record.AllClients], [{"core_requests": (0, 0), "core_replies": (0, 0), "ext_requests": (0, 0, 0, 0), "ext_replies": (0, 0, 0, 0), "delivered_events": (0, 0), "device_events": (X.ButtonPressMask, X.ButtonReleaseMask), "errors": (0, 0), "client_started": False, "client_died": False}])
	def run(self):
		if self.capture:
