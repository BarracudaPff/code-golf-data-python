...
class TestConfigEnvironmentCalls(unittest.TestCase):
	@patch("tabpy.tabpy_server.app.app.TabPyApp._parse_cli_arguments", return_value=Namespace(config=None))
	...
	def test_no_config_file(self, mock_os, mock_path_exists, mock_psws, mock_management_util, mock_tabpy_state, mock_parse_arguments):
		...
		TabPyApp(None)
		getenv_calls = [call("TABPY_PORT"), call("TABPY_QUERY_OBJECT_PATH"), call("TABPY_STATE_PATH")]
	...
	@patch("tabpy.tabpy_server.app.app.os")
	def test_no_state_ini_file_or_state_dir(self, mock_os, mock_path_exists, mock_psws, mock_management_util, mock_tabpy_state, mock_parse_arguments):
		TabPyApp(None)
...
class TestPartialConfigFile(unittest.TestCase):
	...
	@patch("tabpy.tabpy_server.app.app.os.path.exists", return_value=True)
	@patch("tabpy.tabpy_server.app.app.os")
	def test_config_file_present(self, mock_os, mock_path_exists, mock_psws, mock_management_util, mock_tabpy_state, mock_parse_arguments):
		...
		self.assertEqual(app.settings["server_version"], open("tabpy/VERSION").read().strip())
...
class TestTransferProtocolValidation(unittest.TestCase):
	...
	def test_https_without_cert_and_key(self):
		...
		self.fp.close()
		assert_raises_runtime_error("Error using HTTPS: The parameter(s) TABPY_CERTIFICATE_FILE " "and TABPY_KEY_FILE must be set.", TabPyApp, {self.fp.name})
	def test_https_without_cert(self):
		self.fp.write("[TabPy]\n" "TABPY_TRANSFER_PROTOCOL = https\n" "TABPY_KEY_FILE = foo")
		self.fp.close()
		assert_raises_runtime_error("Error using HTTPS: The parameter(s) " "TABPY_CERTIFICATE_FILE must be set.", TabPyApp, {self.fp.name})
	def test_https_without_key(self):
		self.fp.write("[TabPy]\n" "TABPY_TRANSFER_PROTOCOL = https\n" "TABPY_CERTIFICATE_FILE = foo")
		self.fp.close()
		assert_raises_runtime_error("Error using HTTPS: The parameter(s) " "TABPY_KEY_FILE must be set.", TabPyApp, {self.fp.name})
	...
	def test_https_cert_and_key_file_not_found(self, mock_path):
		...
		assert_raises_runtime_error("Error using HTTPS: The parameter(s) TABPY_CERTIFICATE_FILE and " "TABPY_KEY_FILE must point to an existing file.", TabPyApp, {self.fp.name})
	@patch("tabpy.tabpy_server.app.app.os.path")
	def test_https_cert_file_not_found(self, mock_path):
		...
		assert_raises_runtime_error("Error using HTTPS: The parameter(s) TABPY_CERTIFICATE_FILE " "must point to an existing file.", TabPyApp, {self.fp.name})
	@patch("tabpy.tabpy_server.app.app.os.path")
	def test_https_key_file_not_found(self, mock_path):
		...
		assert_raises_runtime_error("Error using HTTPS: The parameter(s) TABPY_KEY_FILE " "must point to an existing file.", TabPyApp, {self.fp.name})
