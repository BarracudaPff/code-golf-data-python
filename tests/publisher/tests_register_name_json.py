...
class PostRegisterNameJsonNotAuth(BaseTestCases.EndpointLoggedOut):
	def setUp(self):
		...
		super().setUp(snap_name=None, endpoint_url=endpoint_url, method_endpoint="POST")
class PostRegisterNameJson(BaseTestCases.EndpointLoggedIn):
	def setUp(self):
		...
		super().setUp(snap_name=None, api_url=api_url, endpoint_url=endpoint_url, method_api="POST", method_endpoint="POST", data=data)
