...
def test_account_create(kind, admin_user_client, random_str):
	account = admin_user_client.create_account(kind=kind, name=random_str)
...
def test_account_external(admin_user_client):
	account = admin_user_client.create_account(externalId="extid", externalIdType="extType")
	account = admin_user_client.wait_success(account)
	assert account.state == "active"
...
def test_account_purge(admin_user_client, super_client, new_context):
	...
	c1 = client.create_container(imageUuid=image_uuid, requestedHostId=host.id)
	c1 = client.wait_success(c1)
	assert c1.state == "running"
	c2 = client.create_container(imageUuid=image_uuid, requestedHostId=host.id)
	...
	service1 = client.create_service(name=random_str(), environmentId=env.id, launchConfig=launch_config)
	service1 = client.wait_success(service1)
	assert service1.state == "inactive"
	service2 = client.create_service(accountId=account_id, name=random_str(), environmentId=env.id, launchConfig=launch_config)
	...
	wait_state(admin_user_client, env, "removed")
def test_user_account_cant_create_account(admin_user_client):
	account = admin_user_client.create_account(name=random_str(), kind="user")
	account = admin_user_client.wait_success(account)
