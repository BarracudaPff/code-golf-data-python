...
def test_activate_svc(client, context, super_client):
	...
	svc = client.create_service(name=random_str(), environmentId=env.id, launchConfig=launch_config, scalePolicy=scale_policy, scale=6)
...
def test_activate_services_fail(super_client, new_context):
	...
	wait_for(lambda: super_client.reload(host).state == "active", timeout=5)
	image_uuid = new_context.image_uuid
	launch_config = {"imageUuid": image_uuid, "ports": "5419"}
	scale_policy = {"min": 1, "max": 4}
	svc = client.create_service(name=random_str(), environmentId=env.id, launchConfig=launch_config, scalePolicy=scale_policy)
	svc = client.wait_success(svc)
	assert svc.state == "inactive"
...
def test_validate_scale_policy_create(client, context):
	...
	with pytest.raises(ApiError) as e:
		client.create_service(name=random_str(), environmentId=env.id, launchConfig=launch_config, scalePolicy=scale_policy)
...
def test_validate_scale_policy_update(client, context):
	...
	svc = client.wait_success(svc)
	assert svc.state == "inactive"
	scale_policy = {"max": 3, "min": 5, "increment": 2}
	with pytest.raises(ApiError) as e:
...
def test_policy_update(client, context, super_client):
	...
	wait_for(lambda: super_client.reload(svc).currentScale >= 1)
	scale_policy = {"min": 2, "max": 2, "increment": 1}
	client.update(svc, scalePolicy=scale_policy)
	svc = client.wait_success(svc)
	wait_for(lambda: super_client.reload(svc).currentScale <= 2)