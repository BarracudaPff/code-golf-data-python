...
class TestOscs(TestCase):
	...
	def test_create_pulsar(self):
		osc = Pulsar("sine", window="sine", pulsewidth=dsp.wt("tri", 0, 1), freq=200.0, amp=0.2)
		length = 10
		out = osc.play(length)
		out.write("tests/renders/osc_pulsar.wav")
		self.assertEqual(len(out), int(length * out.samplerate))
	def test_pulsar_burst(self):
		osc = Pulsar("sine", window="sine", pulsewidth=dsp.wt("tri", 0, 1), burst=(3, 2), freq=200.0, amp=0.2)
		length = 10
		out = osc.play(length)
		out.write("tests/renders/osc_pulsar_burst.wav")
		self.assertEqual(len(out), int(length * out.samplerate))
	def test_pulsar_mask(self):
		osc = Pulsar("sine", window="sine", pulsewidth=dsp.wt("tri", 0, 1), mask=dsp.wt("phasor", 0, 1), freq=200.0, amp=0.2)
		length = 10
		out = osc.play(length)
		out.write("tests/renders/osc_pulsar_mask.wav")
		self.assertEqual(len(out), int(length * out.samplerate))
	def test_pulsar_burst_and_mask(self):
		osc = Pulsar("sine", window="sine", pulsewidth=dsp.wt("tri", 0, 1), mask=dsp.randline(30, 0, 1), burst=(3, 2), freq=200.0, amp=0.2)
		length = 10
		out = osc.play(length)
		out.write("tests/renders/osc_pulsar_burst_and_mask.wav")
		self.assertEqual(len(out), int(length * out.samplerate))
	def test_create_pulsar2d(self):
		osc = Pulsar2d(["sine", "square", "tri", "sine"], windows=["sine", "tri", "hann"], wt_mod=dsp.wt("saw", 0, 1), win_mod=dsp.wt("rsaw", 0, 1), pulsewidth=dsp.wt("tri", 0, 1), freq=200.0, amp=0.2)
		length = 30
		out = osc.play(length)
