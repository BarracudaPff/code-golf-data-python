RESOURCE_DIR = os.path.join(os.path.dirname(os.path.realpath(__file__)), "resources/certs")
def test_create_cert_basic(client):
	cert = _read_cert("san_domain_com.crt")
	key = _read_cert("san_domain_com.key")
	cert1 = client.create_certificate(name=random_str(), cert=cert, key=key)
...
def test_dup_names(super_client, client):
	...
	key = _read_cert("san_domain_com.key")
	name = random_str()
	cert1 = super_client.create_certificate(name=name, cert=cert_input, key=key)
	super_client.wait_success(cert1)
	assert cert1.name == name
	cert2 = client.create_certificate(name=name, cert=cert_input, key=key)
	cert2 = super_client.wait_success(cert2)
	assert cert2.name == name
	assert cert2.accountId != cert1.accountId
	with pytest.raises(ApiError) as e:
		super_client.create_certificate(name=name, cert=cert_input, key=key)
...
def test_create_cert_invalid_cert(client):
	...
	with pytest.raises(ApiError) as e:
		client.create_certificate(name=random_str(), cert=cert, key=key)
	assert e.value.error.status == 422
...
def test_create_cert_chain(client):
	...
	cert1 = client.create_certificate(name=random_str(), cert=cert, key=key, certChain=chain)
	cert1 = client.wait_success(cert1)
	assert cert1.state == "active"
	assert cert1.cert == cert
	return cert1
...
def test_update_cert(client):
	...
	c1 = client.create_certificate(name=random_str(), cert=cert1, key=key1)
