...
def made_log(object, admin_user_client, context, accountId=None):
	...
	if t == "stack":
		t = "stack"
	logs = admin_user_client.list_audit_log(resourceId=object.id, resourceType=t)
	assert len(logs) == 1
...
def test_audit_entry_created(new_context, admin_user_client):
	...
	made_log(new_context.user_client.create_project(), admin_user_client, new_context, accountId=new_context.account.id)
	new_context.user_client._headers = new_headers
	new_context.user_client.reload_schema()
	objects.append(new_context.user_client.create_container(imageUuid=new_context.image_uuid))
	objects.append(new_context.user_client.create_container(imageUuid=new_context.image_uuid))
	objects.append(new_context.user_client.create_api_key())
	objects.append(new_context.user_client.create_registry(serverAddress="test.io", name="test"))
	objects.append(new_context.user_client.create_api_key())
	objects.append(new_context.user_client.create_stack(name="env-" + random_str()))
