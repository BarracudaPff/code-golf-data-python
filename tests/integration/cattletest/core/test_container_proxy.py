...
def test_service_proxy(client, context):
	...
	service = create_and_activate(client, "service", name=random_str(), launchConfig={"imageUuid": "docker:nginx"}, stackId=env.id)
...
def test_container_proxy_labels(context):
	labels = {"io.rancher.websocket.proxy.port": 1234, "io.rancher.websocket.proxy.scheme": "https"}
