...
def test_link_instance_stop_start(super_client, client, context):
	...
	c = context.create_container(instanceLinks={"target1_link": target1.id, "target2_link": target2.id})
...
def test_link_update(client, context):
	...
	c = context.create_container(instanceLinks={"target1_link": target1.id})
...
def test_link_remove(client, context):
	target1 = context.create_container()
	c = client.create_container(imageUuid=context.image_uuid, startOnCreate=False, instanceLinks={"target1_link": target1.id})
...
def test_null_links(context):
	c = context.create_container(instanceLinks={"null_link": None})
	links = c.instanceLinks()
	assert len(links) == 1
...
def test_link_timeout(super_client, client, context):
	t = client.create_container(imageUuid=context.image_uuid, startOnCreate=False)
	c = super_client.create_container(accountId=context.project.id, imageUuid=context.image_uuid, instanceLinks={"t": t.id}, data={"linkWaitTime": 100})
