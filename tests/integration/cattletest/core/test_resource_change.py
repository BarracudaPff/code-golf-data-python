...
SUB_OPTIONS = "?include=hosts&include=instances&include=instanceLinks" "&include=ipAddresses&eventNames=resource.change"
...
def collect_events(client, array):
	...
	def collect():
		...
		try:
			ws = create_connection(sub_url + SUB_OPTIONS, headers=auth_header(client))
			while True:
				line = ws.recv()
				if should_stop:
					break
...
def test_events(admin_client, client, sim_context, request):
	...
	c = admin_client.create_container(imageUuid=sim_context["imageUuid"], requestedHostId=sim_context["host"].id)
