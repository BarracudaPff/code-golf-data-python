...
class ScannedObject(object):
	...
	def to_list(self):
		...
		return ["0x{0:04X}".format(self.origin), self.function_name, self.name, helper.to_hex(self.expression_address)]
	def __eq__(self, other):
		return self.func_ea == other.func_ea and self.name == other.name and self.expression_address == other.expression_address
	def __hash__(self):
	...
	def apply_type(self, tinfo):
		...
		if hx_view:
			...
			if lvar:
				...
				logger.warn("Failed to find previously scanned local variable {} from {}".format(self.name, helper.to_hex(self.expression_address)))
class ScannedStructureMemberObject(ScannedObject):
	...
	def apply_type(self, tinfo):
		if self._applicable:
			logger.warn("Changing type of structure field is not yet implemented. Address - {}".format(helper.to_hex(self.expression_address)))
class SearchVisitor(api.ObjectVisitor):
	...
	def _manipulate(self, cexpr, obj):
		...
		if member:
			logger.debug("\tCreating member with type {}, {}, offset - {}".format(member.type_name, member.scanned_variables, member.offset))
			self.__temporary_structure.add_row(member)
	def _get_member(self, offset, cexpr, obj, tinfo=None, obj_ea=None):
		cexpr_ea = helper.find_asm_address(cexpr, self.parents)
		if offset < 0:
			logger.error("Considered to be impossible: offset - {}, obj - {}".format(offset, helper.to_hex(cexpr_ea)))
			raise AssertionError
