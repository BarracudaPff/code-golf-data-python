...
class DynamoMediaDB(MediaDB):
	...
	def list_media_files(self, startswith=None, media_type=None, label=None):
		...
		if startswith is not None:
			filter_expression = self._add_to_filter_expression(filter_expression, Attr("name").begins_with(startswith))
		if media_type is not None:
			filter_expression = self._add_to_filter_expression(filter_expression, Attr("type").eq(media_type))
		if label is not None:
			filter_expression = self._add_to_filter_expression(filter_expression, Attr("labels").contains(label))
