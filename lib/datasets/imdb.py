class imdb(object):
	...
	def _get_widths(self):
		return [PIL.Image.open(self.image_path_at(i)).size[0] for i in xrange(self.num_images)]
	def append_flipped_images(self):
		...
		for i in xrange(num_images):
			...
			entry = {"boxes": boxes, "gt_overlaps": self.roidb[i]["gt_overlaps"], "gt_classes": self.roidb[i]["gt_classes"], "flipped": True}
	...
	def evaluate_recall(self, candidate_boxes=None, thresholds=None, area="all", limit=None):
		...
		area_ranges = [[0 ** 2, 1e5 ** 2], [0 ** 2, 32 ** 2], [32 ** 2, 96 ** 2], [96 ** 2, 1e5 ** 2], [96 ** 2, 128 ** 2], [128 ** 2, 256 ** 2], [256 ** 2, 512 ** 2], [512 ** 2, 1e5 ** 2]]
	...
	def create_roidb_from_box_list(self, box_list, gt_roidb):
		...
		for i in xrange(self.num_images):
			...
			roidb.append({"boxes": boxes, "gt_classes": np.zeros((num_boxes,), dtype=np.int32), "gt_overlaps": overlaps, "flipped": False, "seg_areas": np.zeros((num_boxes,), dtype=np.float32)})
		return roidb
	@staticmethod
