	...
	for i in range(len(inp)):
		...
		if acc_bits >= bit_len:
			...
			for x in range(byte_pad, out_width):
				out[j + x] = (acc_value >> (acc_bits + (8 * (out_width - x - 1)))) & ((bit_len_mask >> (8 * (out_width - x - 1))) & 0xFF)
			j += out_width
	return out
	...
	for i in range(out_len):
		if acc_bits < 8:
			...
			for x in range(byte_pad, in_width):
				acc_value = acc_value | ((inp[j + x] & ((bit_len_mask >> (8 * (in_width - x - 1))) & 0xFF)) << (8 * (in_width - x - 1)))
			j += in_width
			acc_bits += bit_len
		acc_bits -= 8
...
def gbp_basic(digest, n, k):
	...
	for i in range(0, 2 ** (collision_length + 1)):
		...
		X.append((expand_array(bytearray(tmp_hash[r * n / 8 : (r + 1) * n / 8]), hash_length, collision_length), (i,)))
	for i in range(1, k):
		if DEBUG:
			print("Round %d:" % i)
		if DEBUG:
	...
	while len(X) > 0:
		j = 1
		while j < len(X):
			if not (has_collision(X[-1][0], X[-1 - j][0], k, collision_length) and has_collision(X[-1][0], X[-1 - j][0], k + 1, collision_length)):
				break
			j += 1
		for l in range(0, j - 1):
			for m in range(l + 1, j):
