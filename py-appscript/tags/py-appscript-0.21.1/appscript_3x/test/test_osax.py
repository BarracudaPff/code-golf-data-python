class TC_OSAX(unittest.TestCase):
	def test_1(self):
		...
		self.assertEqual(mactypes.Alias("/Library/Scripts/"), sa.path_to(osax.k.scripts_folder, from_=osax.k.local_domain))
	...
	def test_4(self):
		...
		self.assertEqual(mactypes.Alias("/System/Library/CoreServices/Finder.app/"), sa.path_to(None))
	def test_5(self):
		p = subprocess.Popen("top -l1 | grep Finder | awk '{ print $1 }'", shell=True, stdout=subprocess.PIPE)
		p.wait()
