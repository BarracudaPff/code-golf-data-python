...
class Specifier(Query):
	...
	def AEM_packself(self, codecs):
		...
		self.AEM_packself = lambda codecs: desc
		return desc
class InsertionSpecifier(Specifier):
	...
	def _packself(self, codecs):
		return packlistas(kae.typeInsertionLoc, [(kae.keyAEObject, self._container.AEM_packself(codecs)), (kae.keyAEPosition, self._key)])
...
class _PositionSpecifier(Specifier):
	...
	def _packself(self, codecs):
		return packlistas(kae.typeObjectSpecifier, [(kae.keyAEDesiredClass, packtype(self.AEM_want)), (kae.keyAEKeyForm, self._kKeyForm), (kae.keyAEKeyData, self._packkey(codecs)), (kae.keyAEContainer, self._container.AEM_packself(codecs))])
	def gt(self, val):
...
class AllElements(_MultipleElements):
	...
	def AEM_resolve(self, obj):
		return self._container.AEM_resolve(obj)
class UnkeyedElements(Specifier):
	...
	def AEM_packself(self, codecs):
		return self._container.AEM_packself(codecs)
	def AEM_resolve(self, obj):
...
class _ComparisonTest(Test):
	...
	def AEM_packself(self, codecs):
		return packlistas(kae.typeCompDescriptor, [(kae.keyAEObject1, codecs.pack(self._operand1)), (kae.keyAECompOperator, self._operator), (kae.keyAEObject2, codecs.pack(self._operand2))])
...
class IsIn(Contains):
	_name = "isin"
	def AEM_packself(self, codecs):
		return packlistas(kae.typeCompDescriptor, [(kae.keyAEObject1, codecs.pack(self._operand2)), (kae.keyAECompOperator, self._operator), (kae.keyAEObject2, codecs.pack(self._operand1))])
class _LogicalTest(Test):
	...
	def AEM_packself(self, codecs):
		return packlistas(kae.typeLogicalDescriptor, [(kae.keyAELogicalOperator, self._operator), (kae.keyAELogicalTerms, codecs.pack(self._operands))])
