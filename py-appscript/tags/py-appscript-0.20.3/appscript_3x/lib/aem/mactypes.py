...
class Alias(_Base):
	...
	def __init__(self, path):
		...
		if path is _kNoPath:
			...
			urldesc = newdesc(kae.typeFileURL, convertpathtourl(abspath(path), kCFURLPOSIXPathStyle).encode("utf8"))
			try:
...
class Units:
	...
	def __eq__(self, val):
		return self is val or (self.__class__ == val.__class__ and self._value == val.value and self._type == val.type)
	def __ne__(self, val):
		return not self == val
	def __hash__(self):
