	...
	if compat.PY34:
		def __del__(self):
			if not self._log_traceback:
				return
			exc = self._exception
			context = {"message": ("%s exception was never retrieved" % self.__class__.__name__), "exception": exc, "future": self}
			if self._source_traceback:
...
def wrap_future(future, *, loop=None):
	...
	if isfuture(future):
		return future
	assert isinstance(future, concurrent.futures.Future), "concurrent.futures.Future is expected, got {!r}".format(future)
	if loop is None:
