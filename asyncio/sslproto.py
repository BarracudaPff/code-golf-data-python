...
class _SSLProtocolTransport(transports._FlowControlMixin, transports.Transport):
	...
	def write(self, data):
		...
		if not isinstance(data, (bytes, bytearray, memoryview)):
			raise TypeError("data: expecting a bytes-like instance, got {!r}".format(type(data).__name__))
		if not data:
			return
...
class SSLProtocol(protocols.Protocol):
    ...
    """
	def __init__(self, loop, app_protocol, sslcontext, waiter, server_side=False, server_hostname=None, call_connection_made=True):
		if ssl is None:
		...
		self._loop = loop
		self._app_protocol = app_protocol
		self._app_transport = _SSLProtocolTransport(self._loop, self, self._app_protocol)
		self._sslpipe = None
	...
	def data_received(self, data):
		...
		try:
			...
			if self._loop.get_debug():
				logger.warning("%r: SSL error %s (reason %s)", self, e.errno, e.reason)
			self._abort()
			return
		...
		try:
			...
			if self._loop.get_debug():
				if isinstance(exc, ssl.CertificateError):
					logger.warning("%r: SSL handshake failed " "on verifying the certificate", self, exc_info=True)
				else:
