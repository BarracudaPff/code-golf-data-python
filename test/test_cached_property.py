...
def test_objects_with_cached_properties_can_be_garbage_collected():
	...
	obj.some_property
	assert any(id(tracked_obj) == ident for tracked_obj in gc.get_objects()), "The object is not being tracked by the garbage collector"
	assert not dealloc_flag[0], "The object was already deallocated"
	del obj
	gc.collect()
	assert not any(id(tracked_obj) == ident for tracked_obj in gc.get_objects()), "The object is still being tracked by the garbage collector"
