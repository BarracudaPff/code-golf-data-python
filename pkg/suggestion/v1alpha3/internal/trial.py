...
class Trial(object):
	...
	def convertTrial(trial):
		...
		target_metric, additional_metrics = Metric.convert(trial.status.observation, metric_name)
		trial = Trial(trial.name, assignments, target_metric, metric_name, additional_metrics)
		return trial
	def __str__(self):
		if self.name == None:
			return "Trial(assignment: {})".format(", ".join([str(e) for e in self.assignments]))
		else:
			return "Trial(assignment: {}, metric_name: {}, metric: {}, additional_metrics: {})".format(", ".join([str(e) for e in self.assignments]), self.metric_name, self.target_metric, ", ".join(str(e) for e in self.additional_metrics))
	...
	def generate(list_of_assignments):
		...
		for assignments in list_of_assignments:
			...
			for assignment in assignments:
				buf.append(api.ParameterAssignment(name=assignment.name, value=str(assignment.value)))
