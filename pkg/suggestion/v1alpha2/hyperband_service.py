...
class HyperbandService(api_pb2_grpc.SuggestionServicer):
	...
	def _get_algorithm_settings(self, experiment_name):
		channel = grpc.beta.implementations.insecure_channel(self.manager_addr, self.manager_port)
		with api_pb2.beta_create_Manager_stub(channel) as client:
			alg = client.GetAlgorithmExtraSettings(api_pb2.GetAlgorithmExtraSettingsRequest(experiment_name=experiment_name), 10)
	...
	def GetSuggestions(self, request, context):
		...
		try:
			...
			self.logger.error("Fail to generate trials: \n%s", traceback.format_exc(), extra={"experiment_name": experiment_name})
			raise e
	...
	def _new_hbParameters(self, sParams):
		...
		sParams["current_i"] = 0
		if sParams["current_s"] >= 0:
			sParams["n"] = int(math.ceil(float(sParams["sMax"] + 1) * (float(sParams["eta"] ** sParams["current_s"]) / float(sParams["current_s"] + 1))))
			sParams["r"] = sParams["r_l"] * sParams["eta"] ** (-sParams["current_s"])
	def _parse_suggestionParameters(self, experiment, alg_settings):
		sParams = {"eta": 3, "sMax": -1, "r_l": -1, "b_l": -1, "r": -1, "n": -1, "current_s": -2, "current_i": -1, "resourceName": "", "evaluatingTrials": 0}
		for k, v in alg_settings.items():
			if k in ["eta", "r_l", "b_l"]:
				...
				self.logger.info("Unknown HyperBand Param %s, ignore it", k, extra={"experiment_name": experiment.name})
		if sParams["current_s"] == -1:
			self.logger.info("HyperBand outlerloop has finished.", extra={"experiment_name": experiment.name})
			return sParams
		if sParams["eta"] <= 0:
			sParams["eta"] = 3
		if sParams["sMax"] < 0:
	...
	def _make_bracket(self, experiment, sParams):
		...
		self.logger.info("HyperBand Param eta %d.", sParams["eta"], extra={"experiment_name": experiment.name})
		self.logger.info("HyperBand Param R %d.", sParams["r_l"], extra={"experiment_name": experiment.name})
		self.logger.info("HyperBand Param sMax %d.", sParams["sMax"], extra={"experiment_name": experiment.name})
		self.logger.info("HyperBand Param B %d.", sParams["b_l"], extra={"experiment_name": experiment.name})
		self.logger.info("HyperBand Param n %d.", sParams["n"], extra={"experiment_name": experiment.name})
		self.logger.info("HyperBand Param r %d.", sParams["r"], extra={"experiment_name": experiment.name})
		self.logger.info("HyperBand Param s %d.", sParams["current_s"], extra={"experiment_name": experiment.name})
		self.logger.info("HyperBand Param i %d.", sParams["current_i"], extra={"experiment_name": experiment.name})
		self.logger.info("HyperBand evaluating trials count %d.", sParams["evaluatingTrials"], extra={"experiment_name": experiment.name})
		if sParams["evaluatingTrials"] == 0:
			self._new_hbParameters(sParams)
		return trialSpecs
	def _make_child_bracket(self, experiment, sParams):
		...
		self.logger.info("Generate %d trials by child bracket.", top_trials_num, extra={"experiment_name": experiment.name})
		return trialSpecs
	...
	def _get_top_trial(self, latest_trials_num, top_trials_num, experiment):
		...
		for t in latest_trials:
			if t.status.condition != api_pb2.TrialStatus.TrialConditionType.SUCCEEDED:
				raise Exception("There are some trials which are not completed yet for experiment %s." % experiment.name)
	...
	def _copy_trials(self, trials, r_i, resourceName):
		...
		for t in trials:
			...
			for assignment in t.spec.parameter_assignments.assignments:
				...
				trial_spec.parameter_assignments.assignments.add(name=assignment.name, value=value)
			trialSpecs.append(trial_spec)
		return trialSpecs
	def _make_master_bracket(self, experiment, sParams):
		n = sParams["n"]
		r = int(sParams["r"])
		parameter_config = parsing_util.parse_parameter_configs(experiment.spec.parameter_specs.parameters)
		trial_specs = []
		for _ in range(n):
			sample = parameter_config.random_sample()
			suggestion = parsing_util.parse_x_next_vector(sample, parameter_config.parameter_types, parameter_config.names, parameter_config.discrete_info, parameter_config.categorical_info)
			trial_spec = api_pb2.TrialSpec()
		...
		self.logger.info("Generate %d trials by master bracket.", n, extra={"experiment_name": experiment.name})
		return trial_specs
	def _update_algorithm_extrasettings(self, experiment_name, sParams):
		...
		channel = grpc.beta.implementations.insecure_channel(self.manager_addr, self.manager_port)
		with api_pb2.beta_create_Manager_stub(channel) as client:
			client.UpdateAlgorithmExtraSettings(api_pb2.UpdateAlgorithmExtraSettingsRequest(experiment_name=experiment_name, extra_algorithm_settings=as_list), 10)
	...
	def ValidateAlgorithmSettings(self, request, context):
		...
		if request.experiment_spec.parallel_trial_count < max_parallel:
			return self._set_validate_context_error(context, "parallelTrialCount must be not less than %d." % max_parallel)
		...
		if not valid_resourceName:
			return self._set_validate_context_error(context, "value of resourceName setting must be in parameters.")
		return api_pb2.ValidateAlgorithmSettingsReply()