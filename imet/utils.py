	...
	def __iter__(self):
		...
		if self.num_workers == 0:
			...
			with ThreadPool(processes=self.num_workers) as pool:
				futures = []
				for indices in sample_iter:
					futures.append([pool.apply_async(self._get_item, args=(i,)) for i in indices])
...
def write_event(log, step: int, **data):
	...
	log.write("\n")
	log.flush()
def plot(*args, ymin=None, ymax=None, xmin=None, xmax=None, params=False, max_points=200, legend=True, title=None, print_keys=False, print_paths=False, plt=None, newfigure=True, x_scale=1):
	"""
		...
		for key in sorted(keys):
			...
			if xs:
				...
				if len(xs) > 2 * max_points:
					indices = np.arange(0, len(xs) - 1, len(xs) / max_points).astype(np.int32)
				...
				if ys_err:
					...
					plt.errorbar(xs, ys, yerr=ys_err, fmt="-o", capsize=5, capthick=2, label=label)
				else:
	...
	if title:
		plt.title(title)
	if print_keys:
		print("Found keys: {}".format(", ".join(sorted(all_keys - {"step", "dt"}))))
def _smooth(ys, indices):
	return [np.mean(ys[idx : indices[i + 1]]) for i, idx in enumerate(indices[:-1])]