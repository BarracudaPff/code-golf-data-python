...
import cgi
import re
PULL_REQUEST_FIRSTLINE_RE = re.compile(r"^Merge pull request #(\d+) from (\S+?)/(\S+?)$", re.U)
...
class DevVersion(DownloadableVersion):
	...
	def description_data(self):
		...
		if match:
			...
			additional_data = {"short_descr": short_descr, "pr_url": settings.GIT_PR_URL % pull_id, "pr_id": pull_id, "author_url": settings.GIT_AUTHOR_URL % author, "author": author}
		else:
			additional_data = {"short_descr": lines[0], "author": self.author, "author_url": settings.GIT_AUTHOR_URL % self.author}
	...
	def description_abbrev(self):
		...
		if "pr_id" in data:
			...
			additional_html = additional_html_fmt % {"pr_url": data["pr_url"], "pr_id": data["pr_id"], "author_url": cgi.escape(data["author_url"]), "author": cgi.escape(data["author"])}
		else:
			additional_html = ""
