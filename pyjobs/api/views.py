class DjangoPaginatedResource(DjangoResource):
	...
	def wrap_list_response(self, data):
		...
		if self.page.has_previous() and self.page.previous_page_number():
			previous_page = True
		response_dict["meta"] = {"page": self.page.number, "limit": self.page.paginator.per_page, "total_pages": self.page.paginator.num_pages, "total_count": self.page.paginator.count, "next": next_page, "previous": previous_page}
		return response_dict
class JobResource(DjangoPaginatedResource):
	page_size = 20
	serializer = PyJobsSerializer()
	preparer = FieldsPreparer(fields={field.name: field.name for field in Job._meta.fields if field.name not in {"premium", "public", "ad_interested", "company_email", "cellphone"}})
	def list(self):
