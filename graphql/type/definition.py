...
def is_input_type(type):
	...
	return isinstance(named_type, (GraphQLScalarType, GraphQLEnumType, GraphQLInputObjectType))
def is_output_type(type):
	named_type = get_named_type(type)
	return isinstance(named_type, (GraphQLScalarType, GraphQLObjectType, GraphQLInterfaceType, GraphQLUnionType, GraphQLEnumType))
...
def define_field_map(type, field_map):
	...
	for field_name, field in field_map.items():
		...
		assert isinstance(field, GraphQLField), "{}.{} must be an instance of GraphQLField.".format(type, field_name)
...
class GraphQLArgument(object):
	...
	def __eq__(self, other):
		return self is other or (isinstance(other, GraphQLArgument) and self.type == other.type and self.default_value == other.default_value and self.description == other.description and self.out_name == other.out_name)
	def __hash__(self):
		return id(self)
...
def define_types(union_type, types):
	...
	for type in types:
		assert isinstance(type, GraphQLObjectType), "{} may only contain Object types, it cannot contain: {}.".format(union_type, type)
		if not has_resolve_type_fn:
			assert callable(type.is_type_of), ('Union Type {} does not provide a "resolve_type" function ' 'and possible Type {} does not provide a "is_type_of" ' "function. There is no way to resolve this possible type " "during execution.").format(union_type, type)
	return types
class GraphQLEnumType(GraphQLNamedType):
	...
	def _value_lookup(self):
		return {value.value.value if isinstance(value.value, PyEnum) else value.value: value for value in self.values}
	@cached_property
