...
class DLArray(ctypes.Structure):
	"""DLArray in C API"""
	_fields_ = [("data", ctypes.c_void_p), ("ctx", DLContext), ("ndim", ctypes.c_int), ("shape", ctypes.POINTER(ctypes.c_int64))]
...
class NDArray(object):
	...
	def _sync_copyfrom(self, source_array):
		...
		if not isinstance(source_array, np.ndarray):
			try:
				...
				raise TypeError("array must be an array_like data," + "type %s is not supported" % str(type(source_array)))
		...
		check_call(_LIB.DLArrayCopyFromTo(ctypes.byref(source_arr), self.handle, None))
		_ = shape
	@staticmethod
	...
	def asnumpy(self):
		...
		check_call(_LIB.DLArrayCopyFromTo(self.handle, ctypes.byref(arr), None))
		_ = shape
		return np_arr
	def copyto(self, target):
		...
		if isinstance(target, NDArray):
			check_call(_LIB.DLArrayCopyFromTo(self.handle, target.handle, None))
		else:
...
def empty(shape, ctx=cpu(0)):
	...
	handle = DLArrayHandle()
	check_call(_LIB.DLArrayAlloc(shape, ndim, ctx, ctypes.byref(handle)))
