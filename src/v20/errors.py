class V20ConnectionError(Exception):
	...
	def __init__(self, url):
		self.url = url
	def __str__(self):
		return "Connection to v20 REST server at {} failed".format(self.url)
class V20Timeout(Exception):
	...
	def __init__(self, url, type):
		...
		self.type = type
	def __str__(self):
		return "v20 REST request to {} has timed out ({})".format(self.url, self.type)
class ResponseUnexpectedStatus(Exception):
	...
	def __str__(self):
		return "{} {} expected status {}, got {} ({})".format(self.response.method, self.response.path, self.expected_status, self.response.status, self.response.reason)
class ResponseUnexpectedStatus(Exception):
	"""
    A ResponseUnexpectedStatus exception is raised when a certain HTTP status
    code is expected in a response however a differend status code has
...
class ResponseNoField(Exception):
	...
	def __init__(self, response, field):
		...
		self.field = field
	def __str__(self):
		contains = ", ".join(["'{}'".format(f) for f in self.response.body.keys()])
		return "{} response for {} {} does not have field '{}' (contains {})".format(self.response.status, self.response.method, self.response.path, self.field, contains)