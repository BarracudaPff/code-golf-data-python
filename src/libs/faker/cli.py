	...
	for signature, example in formatters.items():
		...
		for line in lines:
			for i in range(0, (len(line) // remains) + 1):
				...
				signature = separator = " "
def print_doc(provider_or_field=None, args=None, lang=DEFAULT_LOCALE, output=None, seed=None, includes=None):
	args = args or []
	output = output or sys.stdout
...
examples:
  ...
  Gayla Schmitt;
""".format(
			", ".join(sorted(AVAILABLE_LOCALES)), default_locale, DEFAULT_LOCALE
		)
		formatter_class = argparse.RawDescriptionHelpFormatter
		parser = argparse.ArgumentParser(prog=self.prog_name, description="{0} version {1}".format(self.prog_name, VERSION), epilog=epilog, formatter_class=formatter_class)
		...
		parser.add_argument("-o", metavar="output", type=argparse.FileType("w"), default=sys.stdout, help="redirect output to a file")
		parser.add_argument("-l", "--lang", choices=AVAILABLE_LOCALES, default=default_locale, metavar="LOCALE", help="specify the language for a localized " "provider (e.g. de_DE)")
		parser.add_argument("-r", "--repeat", default=1, type=int, help="generate the specified number of outputs")
		parser.add_argument("-s", "--sep", default="\n", help="use the specified separator after each " "output")
		parser.add_argument("--seed", metavar="SEED", type=int, help="specify a seed for the random generator so " "that results are repeatable. Also compatible " "with 'repeat' option")
		parser.add_argument("-i", "--include", default=META_PROVIDERS_MODULES, nargs="*", help="list of additional custom providers to " "user, given as the import path of the module " "containing your Provider class (not the provider " "class itself)")
		parser.add_argument("fake", action="store", nargs="?", help="name of the fake to generate output for " "(e.g. profile)")
		parser.add_argument("fake_args", metavar="fake argument", action="store", nargs="*", help="optional arguments to pass to the fake " "(e.g. the profile fake takes an optional " "list of comma separated field names as the " "first argument)")
		...
		for i in range(arguments.repeat):
			print_doc(arguments.fake, arguments.fake_args, lang=arguments.lang, output=arguments.o, seed=seeds[i], includes=arguments.include)
