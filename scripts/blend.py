...
shape_file = model
setter = model.split("/")
view_params = [[random.uniform(0.0, 12.0) * 360.0 / 12.0, random.uniform(-20, 30.0), 0, 1.25] for i in range(num)]
...
for e, param in enumerate(view_params):
	...
	for i in range(8):
		...
		bpy.data.objects["Point"].data.energy = np.random.normal(0.7, 0.71)
