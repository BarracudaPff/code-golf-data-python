		...
		for f in self.wrappers.functions:
			if not self._ignore(f):
				if option_set("general", "debug"):
					msg = "Patching {} function: {}".format(f["type"], f["function"])
					print(msg)
				setattr(self.__class__, "wrapper", f["wrapper"])
				patch_function(mod, f["function"], self.wrapper)
			else:
				if option_set("general", "debug"):
					print("Ignoring {} for: {}".format(f["type"], self.modulename))
		return mod
	def _ignore(self, f):
...
class WrapperList(object):
	...
	def __init__(self):
		self.functions = []
	def add(self, function_names, log_function, arg_loc, modname, function_type):
		if isinstance(function_names, six.string_types):
			function_names = [function_names]
		wrapper = create_wrapper(log_function, arg_loc, modname)
		for f in function_names:
			self.functions.append({"function": f, "wrapper": wrapper, "type": function_type})
	def add_inputs(self, function_names, log_function, arg_loc, modname):
		return self.add(function_names, log_function, arg_loc, modname, "input")
	def add_outputs(self, function_names, log_function, arg_loc, modname):
		return self.add(function_names, log_function, arg_loc, modname, "output")