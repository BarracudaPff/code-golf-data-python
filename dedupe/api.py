...
class Matching(object):
	...
	def thresholdBlocks(self, blocks, recall_weight=1.5):
		...
		probability = core.scoreDuplicates(candidate_records, self.data_model, self.classifier, self.num_cores)["score"]
	...
	def matchBlocks(self, blocks, threshold=0.5, *args, **kwargs):
        ...
        """
		candidate_records = itertools.chain.from_iterable(self._blockedPairs(blocks))
		matches = core.scoreDuplicates(candidate_records, self.data_model, self.classifier, self.num_cores, threshold=0)
		...
		try:
			...
			pass
	def writeSettings(self, file_obj, index=False):
		"""
...
class DedupeMatching(Matching):
	...
	def match(self, data, threshold=0.5, generator=False):
	...
	def _blockData(self, data_d):
		...
		block_groups = itertools.groupby(self.blocker(viewitems(data_d)), lambda x: x[1])
		...
		blocks = {block_key: record_ids for block_key, record_ids in blocks.items() if len(record_ids) > 1}
		coverage = {record_id: [k for k in cover if k in blocks] for record_id, cover in coverage.items()}
	...
	def _checkBlock(self, block):
		if block:
			try:
				...
				raise ValueError("Each item in a block must be a sequence of " "record_id, record, and smaller ids and the " "records also must be dictionaries")
			try:
				record.items()
				smaller_ids.isdisjoint([])
			except AttributeError:
				raise ValueError("The record must be a dictionary and " "smaller_ids must be a set")
...
class RecordLinkMatching(Matching):
	...
	def match(self, data_1, data_2, threshold=0.5, generator=False):
		"""
	...
	def _blockGenerator(self, messy_data, blocked_records):
		block_groups = itertools.groupby(self.blocker(viewitems(messy_data)), lambda x: x[1])
		for i, (record_id, block_keys) in enumerate(block_groups):
			...
			B = [(rec_id, record, set()) for rec_id, record in B.items()]
			if B:
				yield (A, B)
	...
	def _checkBlock(self, block):
		if block:
			try:
				...
				raise ValueError("Each block must be a made up of two " "sequences, (base_sequence, target_sequence)")
			if base:
				try:
					base_id, base_record, base_smaller_ids = base[0]
				except ValueError:
					raise ValueError("Each sequence must be made up of 3-tuple " "like (record_id, record, covered_blocks)")
...
class StaticMatching(Matching):
    ...
    - __init__
    """
	def __init__(self, settings_file, num_cores=None, **kwargs):
		"""
		...
		try:
			...
			raise SettingsFileLoadingException("This settings file is not compatible with " "the current version of dedupe. This can happen " "if you have recently upgraded dedupe.")
		except:
		...
		for full_predicate in self.predicates:
			for predicate in full_predicate:
				if hasattr(predicate, "index") and predicate.index is None:
					...
					predicate.index._doc_to_id = core.Enumerator(max_id + 1, doc_to_ids[predicate])
					if hasattr(predicate, "canopy"):
...
class ActiveMatching(Matching):
    ...
    - cleanupTraining
    """
	def __init__(self, variable_definition, data_sample=None, num_cores=None, **kwargs):
		"""
	...
	def readTraining(self, training_file):
		...
		training_pairs = json.load(training_file, cls=serializer.dedupe_decoder)
		try:
	...
	def train(self, recall=0.95, index_predicates=True):
		...
		self.blocker = blocking.Blocker(self.predicates)
		self.blocker.resetIndices()
	def writeTraining(self, file_obj):
		"""
        Write to a json file that contains labeled examples
        Keyword arguments:
        file_obj -- file object to write training data to
        """
		json.dump(self.training_pairs, file_obj, default=serializer._to_json, tuple_as_array=False, ensure_ascii=True)
	...
	def _checkTrainingPairs(self, labeled_pairs):
		try:
			...
			raise ValueError("labeled_pairs must be a dictionary with keys " '"distinct" and "match"')
...
class Dedupe(DedupeMatching, ActiveMatching):
    ...
    """
	canopies = True
	def prepare_training(self, data, training_file=None, sample_size=15000, blocked_proportion=0.5, original_length=None):
		"""
        Sets up the learner.
        Arguments:
	...
	def sample(self, data, sample_size=15000, blocked_proportion=0.5, original_length=None):
		...
		examples, y = flatten_training(self.training_pairs)
		self.active_learner = self.ActiveLearner(self.data_model, data, blocked_proportion, sample_size, original_length, index_include=examples)
		self.active_learner.mark(examples, y)
...
class RecordLink(RecordLinkMatching, ActiveMatching):
    ...
    """
	canopies = False
	def prepare_training(self, data_1, data_2, training_file=None, sample_size=15000, blocked_proportion=0.5, original_length_1=None, original_length_2=None):
		"""
        Sets up the learner.
        Arguments:
        data_1      -- Dictionary of records from first dataset, where the
		...
		if training_file:
			self.readTraining(training_file)
		self.sample(data_1, data_2, sample_size, blocked_proportion, original_length_1, original_length_2)
	def sample(self, data_1, data_2, sample_size=15000, blocked_proportion=0.5, original_length_1=None, original_length_2=None):
		"""
		...
		examples, y = flatten_training(self.training_pairs)
		self.active_learner = self.ActiveLearner(self.data_model, data_1, data_2, blocked_proportion, sample_size, original_length_1, original_length_2, index_include=examples)
		self.active_learner.mark(examples, y)
...
class GazetteerMatching(RecordLinkMatching):
	...
	def matchBlocks(self, blocks, threshold=0.5, *args, **kwargs):
		...
		matches = core.scoreGazette(candidate_records, self.data_model, self.classifier, self.num_cores, threshold=threshold)
		logger.debug("matching done, begin clustering")
		return self._cluster(matches, *args, **kwargs)
	def match(self, messy_data, threshold=0.5, n_matches=1, generator=False):
		"""Identifies pairs of records that refer to the same entity, returns
        tuples containing a set of record ids and a confidence score as a float
        between 0 and 1. The record_ids within each set should refer to the
        same entity and the confidence score is the estimated probability that
