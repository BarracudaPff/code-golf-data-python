...
def blockedSample(sampler, sample_size, predicates, *args):
	...
	while remaining_sample and predicates:
		...
		new_sample = list(sampler(remaining_sample, predicates, *args))
		...
		if growth_rate < 0.001:
			logging.debug("%s blocked samples were requested, " "but only able to sample %s" % (sample_size, len(blocked_sample)))
			break
...
def dedupeSamplePredicates(sample_size, predicates, items):
	...
	for subsample_size, predicate in subsample(sample_size, predicates):
		...
		items.reverse()
		yield dedupeSamplePredicate(subsample_size, predicate, items)
