...
def _extract_data(lda_model, dtm, vectorizer):
	...
	err_msg = "Topic-term distributions and document-term matrix" "have different number of columns, {} != {}."
	assert term_freqs.shape[0] == len(vocab), "Term frequencies and vocabulary are of different sizes, {} != {}.".format(term_freqs.shape[0], len(vocab))
	assert topic_term_dists.shape[1] == dtm.shape[1], err_msg.format(topic_term_dists.shape[1], len(vocab))
	doc_topic_dists = _get_doc_topic_dists(lda_model, dtm)
	return {"vocab": vocab, "doc_lengths": doc_lengths.tolist(), "term_frequency": term_freqs.tolist(), "doc_topic_dists": doc_topic_dists.tolist(), "topic_term_dists": topic_term_dists.tolist()}
