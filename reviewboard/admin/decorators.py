...
def superuser_required(view):
	...
	def decorated(request, *args, **kwargs):
		if not request.user.is_authenticated():
			return login(request, template_name="admin/login.html", authentication_form=AdminAuthenticationForm, extra_context={"title": _("Log in"), "app_path": request.get_full_path(), REDIRECT_FIELD_NAME: request.get_full_path()})
		if not (request.user.is_active and request.user.is_superuser):
			return render(request=request, template_name="admin/permission_denied.html", context={"user": request.user})
...
def check_read_only(view):
	...
	def _check_read_only(request, *args, **kwargs):
		if is_site_read_only_for(request.user):
			return HttpResponseRedirect(local_site_reverse("read-only", request=request))
		else:
			return view(request, *args, **kwargs)
