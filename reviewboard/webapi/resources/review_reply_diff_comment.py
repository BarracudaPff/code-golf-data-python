...
class ReviewReplyDiffCommentResource(BaseDiffCommentResource):
	...
	fields = dict({"reply_to": {"type": ResourceFieldType, "resource": ReviewDiffCommentResource, "description": "The comment being replied to."}}, **BaseDiffCommentResource.fields)
	...
	def create(self, request, reply_to_id, *args, **kwargs):
		...
		try:
			...
			return INVALID_FORM_DATA, {"fields": {"reply_to_id": ["This is not a valid comment ID"]}}
		return self.create_or_update_comment_reply(request=request, comment=comment, reply=reply, comments_m2m=reply.comments, default_attrs={"filediff": comment.filediff, "interfilediff": comment.interfilediff, "first_line": comment.first_line, "num_lines": comment.num_lines}, *args, **kwargs)
	@webapi_check_local_site
	@webapi_login_required
