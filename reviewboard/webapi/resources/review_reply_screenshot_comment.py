class ReviewReplyScreenshotCommentResource(BaseScreenshotCommentResource):
	...
	fields = dict({"reply_to": {"type": ResourceFieldType, "resource": ReviewScreenshotCommentResource, "description": "The comment being replied to."}}, **BaseScreenshotCommentResource.fields)
	...
	def create(self, request, reply_to_id, *args, **kwargs):
		...
		try:
			...
			return INVALID_FORM_DATA, {"fields": {"reply_to_id": ["This is not a valid screenshot " "comment ID"]}}
		return self.create_or_update_comment_reply(request=request, comment=comment, reply=reply, comments_m2m=reply.screenshot_comments, default_attrs={"screenshot": comment.screenshot, "x": comment.x, "y": comment.y, "w": comment.w, "h": comment.h}, *args, **kwargs)
	@webapi_check_local_site
	@webapi_login_required
