...
class ReviewFileAttachmentCommentResource(BaseFileAttachmentCommentResource):
	...
	@webapi_request_fields(required=dict({"file_attachment_id": {"type": IntFieldType, "description": "The ID of the file attachment being " "commented on."}}, **BaseFileAttachmentCommentResource.REQUIRED_CREATE_FIELDS), optional=dict({"diff_against_file_attachment_id": {"type": IntFieldType, "description": "The ID of the file attachment that " "``file_attachment_id`` is diffed against. The " "comment applies to the diff between these two " "attachments.", "added_in": "2.0"}}, **BaseFileAttachmentCommentResource.OPTIONAL_CREATE_FIELDS), allow_unknown=True)
	def create(self, request, file_attachment_id=None, diff_against_file_attachment_id=None, *args, **kwargs):
		...
		try:
			...
			return INVALID_FORM_DATA, {"fields": {"file_attachment_id": ["This is not a valid file " "attachment ID"]}}
		...
		if diff_against_file_attachment_id:
			try:
				...
				return INVALID_FORM_DATA, {"fields": {"diff_against_file_attachment_id": ["This is not a valid file attachment ID"]}}
		return self.create_comment(review=review, comments_m2m=review.file_attachment_comments, file_attachment=file_attachment, diff_against_file_attachment=diff_against_file_attachment, fields=("file_attachment", "diff_against_file_attachment"), **kwargs)
	@webapi_check_local_site
	@webapi_login_required
