...
class ResourceListTests(BaseWebAPITestCase):
	...
	def setup_basic_get_test(self, user, with_local_site, local_site_name, populate_items):
		...
		if populate_items:
			...
			items = []
		return (get_screenshot_draft_list_url(review_request, local_site_name), screenshot_draft_list_mimetype, items)
	def setup_basic_post_test(self, user, with_local_site, local_site_name, post_valid_data):
		review_request = self.create_review_request(with_local_site=with_local_site, submitter=user, publish=True)
		return (get_screenshot_draft_list_url(review_request, local_site_name), screenshot_draft_item_mimetype, {"caption": "Trophy", "path": open(self.get_sample_image_filename(), "rb")}, [review_request])
	...
	def test_post_with_permission_denied_error(self):
		...
		with open(self.get_sample_image_filename(), "rb") as f:
			rsp = self.api_post(get_screenshot_draft_list_url(review_request), {"caption": "Trophy", "path": f}, expected_status=403)
...
class ResourceItemTests(BaseWebAPITestCase):
	...
	def setup_basic_delete_test(self, user, with_local_site, local_site_name):
		...
		return (get_screenshot_draft_item_url(review_request, screenshot.pk, local_site_name), [screenshot, review_request])
	...
	def setup_basic_get_test(self, user, with_local_site, local_site_name):
		review_request = self.create_review_request(with_local_site=with_local_site, submitter=user, publish=True)
		screenshot = self.create_screenshot(review_request, draft=True)
		return (get_screenshot_draft_item_url(review_request, screenshot.pk, local_site_name), screenshot_draft_item_mimetype, screenshot)
	def setup_basic_put_test(self, user, with_local_site, local_site_name, put_valid_data):
		review_request = self.create_review_request(with_local_site=with_local_site, submitter=user, publish=True)
		screenshot = self.create_screenshot(review_request, draft=True)
		if put_valid_data:
			put_data = {"caption": "The new caption"}
		else:
			put_data = {}
		return (get_screenshot_draft_item_url(review_request, screenshot.pk, local_site_name), screenshot_draft_item_mimetype, put_data, screenshot, [review_request])
