...
def _compare_item(self, item_rsp, app):
	self.assertEqual(item_rsp["authorization_grant_type"], app.authorization_grant_type)
...
class ResourceListTests(ExtraDataListMixin, BaseWebAPITestCase):
	...
	def setup_basic_get_test(self, user, with_local_site, local_site_name, populate_items):
		if populate_items:
			if with_local_site:
				...
				local_site = None
			items = [Application.objects.create(user=user, local_site=local_site)]
		else:
			items = []
		return (get_oauth_app_list_url(local_site_name=local_site_name), oauth_app_list_mimetype, items)
	...
	def test_get_filtered(self):
		...
		self.assertEqual(applications, self._applications_from_response(rsp["oauth_apps"]))
	@add_fixtures(["test_site"])
	@webapi_test_template
	def test_get_filtered_with_localsite(self):
		...
		applications = self._make_applications(users=[self.user, admin], local_site=local_site, predicate=lambda a: (a.local_site == local_site and a.user == self.user))
	...
	def test_superuser_get(self):
		...
		applications = self._make_applications(users=[self.user, doc], local_site=local_site, predicate=lambda a: a.local_site is None)
		rsp = self.api_get(get_oauth_app_list_url(), {}, expected_mimetype=oauth_app_list_mimetype)
		self.assertIn("stat", rsp)
		self.assertEqual(rsp["stat"], "ok")
		self.assertEqual(applications, self._applications_from_response(rsp["oauth_apps"]))
	...
	def test_superuser_get_local_site(self):
		...
		local_site = LocalSite.objects.get(pk=1)
		doc = User.objects.get(username="doc")
		applications = self._make_applications(users=[self.user, doc], local_site=local_site, predicate=lambda a: a.local_site == local_site)
		rsp = self.api_get(get_oauth_app_list_url(local_site.name), {}, expected_mimetype=oauth_app_list_mimetype)
		self.assertIn("stat", rsp)
		self.assertEqual(rsp["stat"], "ok")
		self.assertEqual(applications, self._applications_from_response(rsp["oauth_apps"]))
	def _applications_from_response(self, item_rsps):
        ...
        """
		return set(Application.objects.filter(pk__in=(item["id"] for item in item_rsps)))
	def _make_applications(self, users, local_site, predicate=None):
        ...
        """
		applications = set()
		applications.update(self.create_oauth_application(u, None, name="%s-app" % u.username) for u in users)
		applications.update(self.create_oauth_application(u, local_site, name="%s-site-app" % u.username) for u in users)
		if predicate:
	...
	def setup_basic_post_test(self, user, with_local_site, local_site_name, post_valid_data):
		if post_valid_data:
			post_data = {"authorization_grant_type": Application.GRANT_CLIENT_CREDENTIALS, "client_type": Application.CLIENT_PUBLIC, "name": "test-application", "redirect_uris": "https://example.com/oauth/"}
		else:
			post_data = {}
		return (get_oauth_app_list_url(local_site_name), oauth_app_item_mimetype, post_data, [])
	...
	def test_post_grant_implicit_no_uris(self):
		...
		self._test_post_redirect_uri_grant_combination(redirect_uris="", grant_type=Application.GRANT_IMPLICIT, is_valid=False)
	@webapi_test_template
	def test_post_grant_implicit_uris(self):
		"""Testing the POST <URL> API with GRANT_IMPLICIT and URIs"""
		self._test_post_redirect_uri_grant_combination(redirect_uris="https://example.com/", grant_type=Application.GRANT_IMPLICIT, is_valid=True)
	@webapi_test_template
	def test_post_grant_authorization_code_no_uris(self):
		"""Testing the POST <URL> API with GRANT_AUTHORIZATION_CODE and no URIs
        """
		self._test_post_redirect_uri_grant_combination(redirect_uris="", grant_type=Application.GRANT_AUTHORIZATION_CODE, is_valid=False)
	@webapi_test_template
	def test_post_grant_authorization_code_uris(self):
		"""Testing the POST <URL> API with GRANT_AUTHORIZATION_CODE and URIs"""
		self._test_post_redirect_uri_grant_combination(redirect_uris="http://example.com", grant_type=Application.GRANT_AUTHORIZATION_CODE, is_valid=True)
	@webapi_test_template
	def test_post_grant_password_no_uris(self):
		"""Testing the POST <URL> API with GRANT_PASSWORD and no URIs"""
		self._test_post_redirect_uri_grant_combination(redirect_uris="", grant_type=Application.GRANT_PASSWORD, is_valid=True)
	@webapi_test_template
	def test_post_grant_password_uris(self):
		"""Testing the POST <URL> API with GRANT_PASSWORD and URIs"""
		self._test_post_redirect_uri_grant_combination(redirect_uris="http://example.com", grant_type=Application.GRANT_PASSWORD, is_valid=True)
	@webapi_test_template
	def test_post_grant_client_credentials_no_uris(self):
		"""Testing the POST <URL> API with GRANT_CLIENT_CREDENTIALS and no URIs
        """
		self._test_post_redirect_uri_grant_combination(redirect_uris="", grant_type=Application.GRANT_CLIENT_CREDENTIALS, is_valid=True)
	@webapi_test_template
	def test_post_grant_client_credentials_uris(self):
		"""Testing the POST <URL> API with GRANT_CLIENT_CREDENTIALS and URIs"""
		self._test_post_redirect_uri_grant_combination(redirect_uris="http://example.com", grant_type=Application.GRANT_CLIENT_CREDENTIALS, is_valid=True)
	@webapi_test_template
	def test_post_set_user(self):
		"""Testing the POST <URL> API with user set"""
		rsp = self.api_post(get_oauth_app_list_url(), {"authorization_grant_type": Application.GRANT_CLIENT_CREDENTIALS, "client_type": Application.CLIENT_PUBLIC, "name": "test-application", "redirect_uris": "https://example.com/oauth/", "user": "doc"}, expected_status=400)
		self.assertIn("stat", rsp)
		self.assertEqual(rsp["stat"], "fail")
		self.assertIn("fields", rsp)
		self.assertIn("user", rsp["fields"])
		self.assertEqual(rsp["fields"]["user"], ["You do not have permission to set this field."])
	@webapi_test_template
	def test_post_set_user_as_superuser(self):
		"""Testing the POST <URL> API as a superuser with user set"""
		self._login_user(admin=True)
		rsp = self.api_post(get_oauth_app_list_url(), {"authorization_grant_type": Application.GRANT_CLIENT_CREDENTIALS, "client_type": Application.CLIENT_PUBLIC, "name": "test-application", "redirect_uris": "https://example.com/oauth/", "user": "doc"}, expected_mimetype=oauth_app_item_mimetype)
		self.assertIn("stat", rsp)
		self.assertEqual(rsp["stat"], "ok")
		app = Application.objects.get(pk=rsp["oauth_app"]["id"])
		self.compare_item(rsp["oauth_app"], app)
	...
	def test_post_set_user_as_superuser_not_exists(self):
        ...
        """
		self._login_user(admin=True)
		rsp = self.api_post(get_oauth_app_list_url(), {"authorization_grant_type": Application.GRANT_CLIENT_CREDENTIALS, "client_type": Application.CLIENT_PUBLIC, "name": "test-application", "redirect_uris": "https://example.com/oauth/", "user": "foofoo"}, expected_status=400)
		self.assertIn("stat", rsp)
		self.assertEqual(rsp["stat"], "fail")
		self.assertIn("fields", rsp)
		self.assertIn("user", rsp["fields"])
		self.assertEqual(rsp["fields"]["user"], ['The user "foofoo" does not exist.'])
	@add_fixtures(["test_site"])
	@webapi_test_template
	def test_post_set_user_as_local_site_admin(self):
		...
		rsp = self.api_post(get_oauth_app_list_url(self.local_site_name), {"authorization_grant_type": Application.GRANT_CLIENT_CREDENTIALS, "client_type": Application.CLIENT_PUBLIC, "name": "test-application", "redirect_uris": "https://example.com/oauth/", "user": "dopey"}, expected_mimetype=oauth_app_item_mimetype)
		self.assertIn("stat", rsp)
		self.assertEqual(rsp["stat"], "ok")
		app = Application.objects.get(pk=rsp["oauth_app"]["id"])
		self.compare_item(rsp["oauth_app"], app)
	...
	def test_post_set_user_as_local_site_admin_with_non_local_site_user(self):
        ...
        """
		self._login_user(admin=True, local_site=True)
		rsp = self.api_post(get_oauth_app_list_url(self.local_site_name), {"authorization_grant_type": Application.GRANT_CLIENT_CREDENTIALS, "client_type": Application.CLIENT_PUBLIC, "name": "test-application", "redirect_uris": "https://example.com/oauth/", "user": "dopey"}, expected_status=400)
		self.assertIn("stat", rsp)
		self.assertEqual(rsp["stat"], "fail")
		self.assertIn("fields", rsp)
		self.assertIn("user", rsp["fields"])
		self.assertEqual(rsp["fields"]["user"], ['The user "dopey" does not exist.'])
	@webapi_test_template
	def test_post_set_skip_authorization(self):
		"""Testing the POST <URL> API with skip_authorization set"""
		rsp = self.api_post(get_oauth_app_list_url(), {"authorization_grant_type": Application.GRANT_CLIENT_CREDENTIALS, "client_type": Application.CLIENT_PUBLIC, "name": "test-application", "redirect_uris": "https://example.com/oauth/", "skip_authorization": "1"}, expected_status=400)
		self.assertIn("stat", rsp)
		self.assertEqual(rsp["stat"], "fail")
		self.assertIn("fields", rsp)
		self.assertIn("skip_authorization", rsp["fields"])
		self.assertEqual(rsp["fields"]["skip_authorization"], ["You do not have permission to set this field."])
	@webapi_test_template
	def test_post_set_skip_authorization_as_superuser(self):
		"""Testing the POST <URL> API as a superuser with skip_authorization"""
		self._login_user(admin=True)
		rsp = self.api_post(get_oauth_app_list_url(), {"authorization_grant_type": Application.GRANT_CLIENT_CREDENTIALS, "client_type": Application.CLIENT_PUBLIC, "name": "test-application", "redirect_uris": "https://example.com/oauth/", "skip_authorization": "1"}, expected_mimetype=oauth_app_item_mimetype)
		self.assertIn("stat", rsp)
		self.assertEqual(rsp["stat"], "ok")
		app = Application.objects.get(pk=rsp["oauth_app"]["id"])
		self.compare_item(rsp["oauth_app"], app)
	...
	def test_post_set_skip_authorization_as_local_site_admin(self):
        ...
        """
		self._login_user(admin=True, local_site=True)
		rsp = self.api_post(get_oauth_app_list_url(self.local_site_name), {"authorization_grant_type": Application.GRANT_CLIENT_CREDENTIALS, "client_type": Application.CLIENT_PUBLIC, "name": "test-application", "redirect_uris": "https://example.com/oauth/", "skip_authorization": "1"}, expected_mimetype=oauth_app_item_mimetype)
		self.assertIn("stat", rsp)
		self.assertEqual(rsp["stat"], "ok")
		app = Application.objects.get(pk=rsp["oauth_app"]["id"])
		self.compare_item(rsp["oauth_app"], app)
	...
	def _test_post_redirect_uri_grant_combination(self, redirect_uris, grant_type, is_valid):
        ...
        """
		post_data = {"authorization_grant_type": grant_type, "client_type": Application.CLIENT_PUBLIC, "name": "test-app", "redirect_uris": redirect_uris, "skip_authorization": "0"}
		if is_valid:
			rsp = self.api_post(get_oauth_app_list_url(), post_data, expected_mimetype=oauth_app_item_mimetype)
			self.assertIn("stat", rsp)
			self.assertEqual(rsp["stat"], "ok")
			self.compare_item(rsp["oauth_app"], Application.objects.get(name="test-app"))
		else:
...
class ResourceItemTests(ExtraDataItemMixin, BaseWebAPITestCase):
	...
	def setup_basic_get_test(self, user, with_local_site, local_site_name):
		...
		return (get_oauth_app_item_url(app.pk, local_site_name), oauth_app_item_mimetype, app)
	@webapi_test_template
	...
	def setup_basic_put_test(self, user, with_local_site, local_site_name, put_valid_data):
		app = self.create_oauth_application(user, with_local_site=with_local_site)
		if put_valid_data:
			request_data = {"extra_data.fake_key": ""}
		else:
			request_data = {"user": "admin"}
		return (get_oauth_app_item_url(app.pk, local_site_name), oauth_app_item_mimetype, request_data, app, [])
	...
	def test_put_re_enable_security_disabled(self):
		...
		rsp = self.api_put(get_oauth_app_item_url(app.pk, local_site.name), {"enabled": "1"}, expected_status=400)
		app = Application.objects.get(pk=app.pk)
		self.assertIn("stat", rsp)
		self.assertEqual(rsp["stat"], "fail")
		self.assertIn("fields", rsp)
		self.assertIn("__all__", rsp["fields"])
		self.assertEqual(rsp["fields"]["__all__"][0], ApplicationChangeForm.DISABLED_FOR_SECURITY_ERROR)
		self.assertEqual(app.original_user, doc)
	...
	def test_put_regenerate_secret_key(self):
		...
		original_secret = app.client_secret
		rsp = self.api_put(get_oauth_app_item_url(app.pk), {"regenerate_client_secret": 1}, expected_mimetype=oauth_app_item_mimetype)
		app = Application.objects.get(pk=app.pk)
		self.assertIn("stat", rsp)
		self.assertEqual(rsp["stat"], "ok")
		self.compare_item(rsp["oauth_app"], app)
	...
	def test_put_regenerate_secret_key_enable(self):
		...
		self.assertEqual(app.user, self.user)
		self.assertEqual(app.original_user, doc)
		rsp = self.api_put(get_oauth_app_item_url(app.pk, local_site.name), {"enabled": "1", "regenerate_client_secret": "1"}, expected_mimetype=oauth_app_item_mimetype)
		app = Application.objects.get(pk=app.pk)
		self.assertIn("stat", rsp)
		self.assertEqual(rsp["stat"], "ok")
	...
	def setup_basic_delete_test(self, user, with_local_site, local_site_name):
		...
		return (get_oauth_app_item_url(app.pk, local_site_name), [app.pk])
