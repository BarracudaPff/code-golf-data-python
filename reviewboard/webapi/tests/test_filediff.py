...
class ResourceListTests(ReviewRequestChildListMixin, BaseWebAPITestCase):
	...
	sample_api_url = "review-requests/<review-request-id>/diffs/<revision>/files/"
	...
	def setup_basic_get_test(self, user, with_local_site, local_site_name, populate_items):
		...
		return (get_filediff_list_url(diffset, review_request, local_site_name), filediff_list_mimetype, items)
	def setup_review_request_child_test(self, review_request):
		...
		diffset = self.create_diffset(review_request)
		return (get_filediff_list_url(diffset, review_request), filediff_list_mimetype)
	...
	def test_commit_filter(self):
		...
		with override_feature_check(dvcs_feature.feature_id, enabled=True):
			...
			rsp = self.api_get("%s?commit-id=%s" % (get_filediff_list_url(diffset, review_request), commit.commit_id), expected_status=200, expected_mimetype=filediff_list_mimetype)
	...
	def test_commit_filter_no_results(self):
		...
		with override_feature_check(dvcs_feature.feature_id, enabled=True):
			repository = self.create_repository()
			review_request = self.create_review_request(repository=repository, submitter=self.user, create_with_history=True)
			diffset = self.create_diffset(review_request=review_request, repository=repository)
			commit = self.create_diffcommit(diffset=diffset, repository=repository)
			diffset.finalize_commit_series(cumulative_diff=self.DEFAULT_GIT_FILEDIFF_DATA_DIFF, validation_info=None, validate=False, save=True)
			rsp = self.api_get("%s?commit-id=%s" % (get_filediff_list_url(diffset, review_request), commit.parent_id), expected_status=200, expected_mimetype=filediff_list_mimetype)
			self.assertIn("stat", rsp)
			self.assertEqual(rsp["stat"], "ok")
			self.assertIn("files", rsp)
	...
	def test_history_no_commit_filter(self):
		...
		with override_feature_check(dvcs_feature.feature_id, enabled=True):
			...
			rsp = self.api_get(get_filediff_list_url(diffset, review_request), expected_mimetype=filediff_list_mimetype)
			self.assertIn("stat", rsp)
			self.assertEqual(rsp["stat"], "ok")
			self.assertIn("files", rsp)
			self.assertEqual(rsp["total_results"], 1)
...
class ResourceItemTests(ExtraDataItemMixin, ReviewRequestChildItemMixin, BaseWebAPITestCase):
	...
	resource = resources.filediff
	sample_api_url = "review-requests/<review-request-id>/diffs/<revision>/files/" "<file-id>/"
	compare_item = _compare_item
	fixtures = ["test_users", "test_scmtools"]
	test_http_methods = ("GET",)
	...
	def setup_basic_get_test(self, user, with_local_site, local_site_name):
		...
		return (get_filediff_item_url(filediff, review_request, local_site_name), filediff_item_mimetype, filediff)
	def setup_basic_put_test(self, user, with_local_site, local_site_name, put_valid_data):
		repository = self.create_repository(with_local_site)
		review_request = self.create_review_request(with_local_site=with_local_site, repository=repository, submitter=user, publish=True)
		diffset = self.create_diffset(review_request)
		filediff = self.create_filediff(diffset)
		return (get_filediff_item_url(filediff, review_request, local_site_name), filediff_item_mimetype, {"extra_data.test": "foo"}, filediff, [])
	def setup_review_request_child_test(self, review_request):
		"""Set up the review request child tests.
        Args:
            review_request (reviewboard.reviews.models.review_request.
