...
class ResourceTests(BaseWebAPITestCase):
	...
	def test_get(self):
		...
		self.assertEqual(rsp["uri_templates"]["repository"], "http://testserver/api/repositories/{repository_id}/")
		self._check_common_root_fields(rsp)
	@add_fixtures(["test_users", "test_site"])
	def test_get_with_site(self):
		"""Testing the GET / API with local sites"""
		self._login_user(local_site=True)
		rsp = self.api_get(get_root_url("local-site-1"), expected_mimetype=root_item_mimetype)
		self.assertEqual(rsp["stat"], "ok")
		self.assertIn("uri_templates", rsp)
		self.assertIn("repository", rsp["uri_templates"])
		self.assertEqual(rsp["uri_templates"]["repository"], "http://testserver/s/local-site-1/api/" "repositories/{repository_id}/")
		self._check_common_root_fields(rsp)
	@add_fixtures(["test_users", "test_site"])
	...
	def test_get_with_site_and_cache(self):
		...
		rsp = self.api_get(get_root_url("local-site-2"), expected_mimetype=root_item_mimetype)
		self.assertEqual(rsp["stat"], "ok")
		self.assertIn("uri_templates", rsp)
		self.assertIn("repository", rsp["uri_templates"])
		self.assertEqual(rsp["uri_templates"]["repository"], "http://testserver/s/local-site-2/api/" "repositories/{repository_id}/")
