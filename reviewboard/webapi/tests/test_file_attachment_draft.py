...
class ResourceListTests(BaseWebAPITestCase):
	...
	def setup_basic_get_test(self, user, with_local_site, local_site_name, populate_items):
		...
		if populate_items:
			...
			items = []
		return (get_draft_file_attachment_list_url(review_request, local_site_name), draft_file_attachment_list_mimetype, items)
	...
	def setup_basic_post_test(self, user, with_local_site, local_site_name, post_valid_data):
		review_request = self.create_review_request(with_local_site=with_local_site, submitter=user)
		if post_valid_data:
			post_data = {"path": open(self.get_sample_image_filename(), "rb"), "caption": "New caption"}
		else:
			post_data = {}
		return (get_draft_file_attachment_list_url(review_request, local_site_name), draft_file_attachment_item_mimetype, post_data, [review_request])
	...
	def test_post_with_permission_denied_error(self):
		...
		with open(self.get_sample_image_filename(), "rb") as f:
			rsp = self.api_post(get_draft_file_attachment_list_url(review_request), {"caption": "Trophy", "path": f}, expected_status=403)
...
class ResourceItemTests(BaseWebAPITestCase):
	...
	def setup_basic_delete_test(self, user, with_local_site, local_site_name):
		...
		return (get_draft_file_attachment_item_url(review_request, file_attachment.pk, local_site_name), [review_request, file_attachment])
	...
	def test_delete_file_with_publish(self):
        ...
        """
		review_request = self.create_review_request(submitter=self.user, target_people=[self.user])
		file_attachment = self.create_file_attachment(review_request, draft=True)
	...
	def setup_basic_get_test(self, user, with_local_site, local_site_name):
		review_request = self.create_review_request(with_local_site=with_local_site, submitter=user)
		file_attachment = self.create_file_attachment(review_request, draft=True)
		return (get_draft_file_attachment_item_url(review_request, file_attachment.pk, local_site_name), draft_file_attachment_item_mimetype, file_attachment)
	def setup_basic_put_test(self, user, with_local_site, local_site_name, put_valid_data):
		review_request = self.create_review_request(with_local_site=with_local_site, submitter=user)
		file_attachment = self.create_file_attachment(review_request)
		return (get_draft_file_attachment_item_url(review_request, file_attachment.pk, local_site_name), draft_file_attachment_item_mimetype, {"caption": "My new caption"}, file_attachment, [])
	...
	def test_put_with_non_owner_superuser(self):
		...
		user = self._login_user(admin=True)
		self.assertNotEqual(user, self.user)
		rsp = self.api_put(get_draft_file_attachment_item_url(review_request, file_attachment.pk), {"caption": "My new caption"}, expected_mimetype=draft_file_attachment_item_mimetype)
		self.assertEqual(rsp["stat"], "ok")
		self.check_put_result(user, rsp["draft_file_attachment"], file_attachment)
	@add_fixtures(["test_site"])
	def test_put_file_with_non_owner_local_site_admin(self):
		...
		user = self._login_user(local_site=True, admin=True)
		self.assertNotEqual(user, self.user)
		rsp = self.api_put(get_draft_file_attachment_item_url(review_request, file_attachment.pk, self.local_site_name), {"caption": "My new caption"}, expected_mimetype=draft_file_attachment_item_mimetype)
		self.assertEqual(rsp["stat"], "ok")
		self.check_put_result(user, rsp["draft_file_attachment"], file_attachment)