...
class LegacyScreenshotReviewUI(ReviewUI):
	...
	def get_js_model_data(self):
		return {"imageURL": self.obj.image.url, "screenshotID": self.obj.id}
	def serialize_comments(self, comments):
		result = {}
		serialized_comments = super(LegacyScreenshotReviewUI, self).serialize_comments(comments)
	...
	def serialize_comment(self, comment):
		data = super(LegacyScreenshotReviewUI, self).serialize_comment(comment)
		data.update({"x": comment.x, "y": comment.y, "w": comment.w, "h": comment.h})
		return data