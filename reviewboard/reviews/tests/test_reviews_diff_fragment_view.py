...
class ReviewsDiffFragmentViewTests(TestCase):
	...
	def test_base_filediff_not_in_diffset(self):
		...
		rsp = self.client.get(local_site_reverse("view-diff-fragment", kwargs={"review_request_id": review_request.display_id, "revision": diffset.revision, "filediff_id": filediff.pk}), data={"base-filediff-id": other_filediff.pk})
	...
	def test_base_filediff_not_ancestor(self):
        ...
        """
		review_request = self.create_review_request(create_repository=True, create_with_history=True)
		review_request.target_people = [review_request.submitter]
		diffset = self.create_diffset(review_request, draft=True)
		commits = [self.create_diffcommit(diffset=diffset, commit_id="r1", parent_id="r0"), self.create_diffcommit(diffset=diffset, commit_id="r2", parent_id="r1")]
		base_filediff = commits[0].files.get()
		filediff = commits[1].files.get()
		diffset.finalize_commit_series(cumulative_diff=self.DEFAULT_GIT_FILEDIFF_DATA_DIFF, validation_info=None, validate=False, save=True)
		review_request.publish(user=review_request.submitter)
		rsp = self.client.get(local_site_reverse("view-diff-fragment", kwargs={"revision": diffset.revision, "review_request_id": review_request.display_id, "filediff_id": filediff.pk}), data={"base-filediff-id": base_filediff.pk})
		self.assertEqual(rsp.status_code, 500)
		self.assertIn(b"The requested FileDiff (ID %d) is not a valid base FileDiff " b"for FileDiff %d." % (base_filediff.pk, filediff.pk), rsp.content)