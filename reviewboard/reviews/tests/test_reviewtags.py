...
class DisplayReviewRequestTrophiesTests(TestCase):
	...
	def test_new_style_trophy(self):
		...
		t = Template("{% load reviewtags %}" "{% display_review_request_trophies review_request %}")
		request = self._request_factory.get("/")
		request.user = review_request.submitter
		text = t.render(RequestContext(request, {"review_request": review_request}))
...
class ForReviewRequestFieldTests(SpyAgency, TestCase):
	...
	def test_render_instantiated_fields(self):
		...
		try:
			t = Template("{% load reviewtags %}" "{% for_review_request_field review_request fieldset %}" "Never reached." "{% end_for_review_request_field %}")
			result = t.render(Context({"review_request": review_request, "fieldset": TestFieldSet(review_request)}))
		...
		self.assertEqual(logging.exception.spy.calls[0].args[2].args, (exception_id,))
class DiffCommentLineNumbersTests(TestCase):
	...
	def test_delete_single_lines(self):
        ...
        commented line
        """
		t = Template("{% load reviewtags %}" "{% diff_comment_line_numbers chunks comment %}")
		result = t.render(Context({"comment": Comment(first_line=20, num_lines=1), "chunks": [{"change": "delete", "lines": [(10, 20, "deleted line", [], "", "", [], False), (50, 60, "deleted line", [], "", "", [], False)]}]}))
	...
	def test_delete_mutiple_lines(self):
        ...
        """
		t = Template("{% load reviewtags %}" "{% diff_comment_line_numbers chunks comment %}")
		result = t.render(Context({"comment": Comment(first_line=20, num_lines=2), "chunks": [{"change": "delete", "lines": [(10, 20, "deleted line", [], "", "", [], False), (50, 60, "deleted line", [], "", "", [], False)]}]}))
	...
	def test_replace_single_line(self):
        ...
        """
		t = Template("{% load reviewtags %}" "{% diff_comment_line_numbers chunks comment %}")
		result = t.render(Context({"comment": Comment(first_line=20, num_lines=1), "chunks": [{"change": "replace", "lines": [(10, 20, "foo", [], 20, "replaced line", [], False), (50, 60, "foo", [], 60, "replaced line", [], False)]}]}))
	...
	def test_replace_multiple_lines(self):
        ...
        """
		t = Template("{% load reviewtags %}" "{% diff_comment_line_numbers chunks comment %}")
		result = t.render(Context({"comment": Comment(first_line=20, num_lines=2), "chunks": [{"change": "replace", "lines": [(10, 20, "foo", [], 20, "replaced line", [], False), (50, 60, "foo", [], 60, "replaced line", [], False)]}]}))
		self.assertEqual(result, "Lines 30-31 (original), 30-31 (patched)")
	def test_insert_single_line(self):
		"""Testing diff_comment_line_numbers with insert chunk and single
        comented line
        """
		t = Template("{% load reviewtags %}" "{% diff_comment_line_numbers chunks comment %}")
		result = t.render(Context({"comment": Comment(first_line=20, num_lines=1), "chunks": [{"change": "insert", "lines": [(10, "", "", [], 20, "inserted line", [], False), (50, "", "", [], 60, "inserted line", [], False)]}]}))
	...
	def test_insert_multiple_lines(self):
        ...
        """
		t = Template("{% load reviewtags %}" "{% diff_comment_line_numbers chunks comment %}")
		result = t.render(Context({"comment": Comment(first_line=20, num_lines=2), "chunks": [{"change": "insert", "lines": [(10, "", "", [], 20, "inserted line", [], False), (50, "", "", [], 60, "inserted line", [], False)]}]}))
	...
	def test_fake_equal_orig(self):
        ...
        """
		t = Template("{% load reviewtags %}" "{% diff_comment_line_numbers chunks comment %}")
		result = t.render(Context({"comment": Comment(first_line=20, num_lines=2), "chunks": [{"change": "equal", "lines": [(10, "", "", [], 20, "inserted line", [], False), (50, "", "", [], 60, "inserted line", [], False)]}]}))
		self.assertEqual(result, "Lines 30-31 (patched)")
	def test_fake_equal_patched(self):
        ...
        """
		t = Template("{% load reviewtags %}" "{% diff_comment_line_numbers chunks comment %}")
		result = t.render(Context({"comment": Comment(first_line=20, num_lines=2), "chunks": [{"change": "equal", "lines": [(10, 20, "deleted line", [], "", "", [], False), (50, 60, "deleted line", [], "", "", [], False)]}]}))
		self.assertEqual(result, "Lines 30-31 (original)")
	def test_spanning_inserts_deletes(self):
		"""Testing diff_comment_line_numbers with spanning delete and insert"""
		t = Template("{% load reviewtags %}" "{% diff_comment_line_numbers chunks comment %}")
		result = t.render(Context({"comment": Comment(first_line=20, num_lines=50), "chunks": [{"change": "delete", "lines": [(10, 20, "deleted line", [], "", "", [], False), (50, 60, "deleted line", [], "", "", [], False)]}, {"change": "insert", "lines": [(51, "", "", [], 61, "inserted line", [], False), (100, "", "", [], 110, "inserted line", [], False)]}, {"change": "equal", "lines": [(101, 61, "equal line", [], 111, "equal line", [], False), (200, 160, "equal line", [], 210, "equal line", [], False)]}]}))
	...
	def test_spanning_deletes_inserts(self):
		...
		t = Template("{% load reviewtags %}" "{% diff_comment_line_numbers chunks comment %}")
		result = t.render(Context({"comment": Comment(first_line=20, num_lines=50), "chunks": [{"change": "insert", "lines": [(10, "", "", [], 20, "inserted line", [], False), (50, "", "", [], 60, "inserted line", [], False)]}, {"change": "delete", "lines": [(51, 61, "inserted line", [], "", "", [], False), (100, 110, "inserted line", [], "", "", [], False)]}, {"change": "equal", "lines": [(101, 111, "equal line", [], 61, "equal line", [], False), (200, 210, "equal line", [], 160, "equal line", [], False)]}]}))
	...
	def test_spanning_last_chunk(self):
        ...
        """
		t = Template("{% load reviewtags %}" "{% diff_comment_line_numbers chunks comment %}")
		result = t.render(Context({"comment": Comment(first_line=20, num_lines=50), "chunks": [{"change": "delete", "lines": [(10, 20, "deleted line", [], "", "", [], False), (50, 60, "deleted line", [], "", "", [], False)]}, {"change": "insert", "lines": [(51, "", "", [], 61, "inserted line", [], False), (100, "", "", [], 110, "inserted line", [], False)]}]}))
		self.assertEqual(result, "Lines 30-60 (original), 61-79 (patched)")
class CommentRepliesTests(TestCase):
	...
	def _test_diff_comments(self, user_is_owner):
		...
		self._check_replies(review, comment, self.create_diff_comment, {"filediff": filediff}, user_is_owner)
	...
	def _test_file_attachment_comments(self, user_is_owner):
		...
		self._check_replies(review, comment, self.create_file_attachment_comment, {"file_attachment": file_attachment}, user_is_owner)
	def _test_screenshot_comments(self, user_is_owner):
		review_request = self.create_review_request(publish=True)
		screenshot = self.create_screenshot(review_request)
		review = self.create_review(review_request, publish=True)
		comment = self.create_screenshot_comment(review, screenshot)
		self._check_replies(review, comment, self.create_screenshot_comment, {"screenshot": screenshot}, user_is_owner)
	def _check_replies(self, review, comment, create_comment_func, create_comment_kwargs, user_is_owner):
		reply_kwargs = {"review": review, "user": review.user}
		...
		t = Template("{% load reviewtags %}" '{% comment_replies review comment "123" %}')
		request = RequestFactory().request()
		if user_is_owner:
			request.user = review.user
		else:
			request.user = User.objects.create_user(username="test-user", email="user@example.com")
		html = t.render(RequestContext(request, {"comment": comment, "review": review}))
		...
		if user_is_owner:
			self.assertIn('<li class="draft" data-comment-id="%s"' % reply_comment4.pk, html)
		else:
...
class ReviewBodyRepliesTests(TestCase):
	...
	def _test_body_field(self, body_field, user_is_owner):
		review_request = self.create_review_request(publish=True)
		review = self.create_review(review_request, publish=True)
		reply_kwargs = {"review": review, "user": review.user, "%s_reply_to" % body_field: review, body_field: "Some reply"}
		reply1 = self.create_reply(publish=True, **reply_kwargs)
		reply2 = self.create_reply(publish=True, **reply_kwargs)
		reply3 = self.create_reply(publish=False, **reply_kwargs)
		t = Template("{%% load reviewtags %%}" '{%% review_body_replies review "%s" "123" %%}' % body_field)
		request = RequestFactory().request()
		if user_is_owner:
			request.user = review.user
		else:
			request.user = User.objects.create_user(username="test-user", email="user@example.com")
		html = t.render(RequestContext(request, {"review": review}))
