...
class ActionRegistryTests(ActionsTestCase):
	...
	def test_register_actions_with_invalid_parent_id(self):
		...
		message = "bad-id does not correspond to a registered review request action"
		foo_action.register()
	...
	def test_register_actions_with_already_registered_action(self):
		...
		foo_action = FooAction()
		message = "%s already corresponds to a registered review request action" % foo_action.action_id
		foo_action.register()
		with self.assertRaisesMessage(KeyError, message):
	...
	def test_register_actions_with_too_deep(self):
		...
		error_message = "%s exceeds the maximum depth limit of %d" % (invalid_action.action_id, MAX_DEPTH_LIMIT)
	...
	def test_unregister_actions_with_unregistered_action(self):
		...
		foo_action = FooAction()
		error_message = "%s does not correspond to a registered review request action" % foo_action.action_id
...
class BaseReviewRequestActionTests(ActionsTestCase):
	...
	def test_register_with_already_registered(self):
        ...
        """
		foo_action = FooAction()
		error_message = "%s already corresponds to a registered review request action" % foo_action.action_id
		foo_action.register()
		with self.assertRaisesMessage(KeyError, error_message):
			foo_action.register()
	...
	def test_unregister_with_unregistered_action(self):
        ...
        """
		foo_action = FooAction()
		message = "%s does not correspond to a registered review request action" % foo_action.action_id
		with self.assertRaisesMessage(KeyError, message):
			foo_action.unregister()
...
class AddGeneralCommentActionTests(ReadOnlyActionTestsMixin, ActionsTestCase):
	...
	def test_should_render_with_authenticated(self):
        ...
        """
		self.request.user = User.objects.get(username="doc")
		self.assertTrue(self.action.should_render(self._create_request_context()))
	def test_should_render_with_anonymous(self):
		"""Testing AddGeneralCommentAction.should_render with authenticated
        user
        """
		self.request.user = AnonymousUser()
		self.assertFalse(self.action.should_render(self._create_request_context()))
class CloseMenuActionTests(ReadOnlyActionTestsMixin, ActionsTestCase):
	"""Unit tests for CloseMenuAction."""
	action = CloseMenuAction()
	fixtures = ["test_users"]
	def _create_request_context(self, can_change_status=True, public=True, status=ReviewRequest.PENDING_REVIEW, user=None):
		"""Create and return objects for use in the request context.
        Args:
        ...
        Returns:
            ...
            Additional context to use when testing read-only actions.
        """
		review_request = self.create_review_request(public=public, status=status)
		self.request.user = user or review_request.submitter
		return {"review_request": review_request, "request": self.request, "perms": {"reviews": {"can_change_status": can_change_status}}}
	...
	def test_should_render_for_owner_unpublished(self):
        ...
        """
		self.assertTrue(self.action.should_render(self._create_request_context(public=False)))
	def test_should_render_for_user(self):
		"""Testing CloseMenuAction.should_render for normal user"""
		self.assertFalse(self.action.should_render(self._create_request_context(can_change_status=False, user=User.objects.create_user(username="test-user", email="user@example.com"))))
	def test_should_render_user_with_can_change_status(self):
        ...
        """
		self.assertTrue(self.action.should_render(self._create_request_context(can_change_status=True, user=User.objects.create_user(username="test-user", email="user@example.com"))))
	def test_should_render_user_with_can_change_status_and_unpublished(self):
        ...
        """
		self.assertFalse(self.action.should_render(self._create_request_context(can_change_status=True, public=False, user=User.objects.create_user(username="test-user", email="user@example.com"))))
	def test_should_render_with_discarded(self):
		"""Testing CloseMenuAction.should_render with discarded review request
        """
		self.assertFalse(self.action.should_render(self._create_request_context(status=ReviewRequest.DISCARDED)))
	def test_should_render_with_submitted(self):
		"""Testing CloseMenuAction.should_render with submitted review request
        """
		self.assertFalse(self.action.should_render(self._create_request_context(status=ReviewRequest.SUBMITTED)))
class DeleteActionTests(ReadOnlyActionTestsMixin, ActionsTestCase):
	"""Unit tests for DeleteAction."""
	fixtures = ["test_users"]
	action = DeleteAction()
	def _create_request_context(self, delete_reviewrequest=True, *args, **kwargs):
		"""Create and return objects for use in the request context.
        Args:
        ...
        Returns:
            ...
            Additional context to use when testing read-only actions.
        """
		return {"request": self.request, "perms": {"reviews": {"delete_reviewrequest": delete_reviewrequest}}}
	def test_should_render_with_published(self):
		"""Testing DeleteAction.should_render with standard user"""
		self.request.user = User()
		self.assertFalse(self.action.should_render(self._create_request_context(delete_reviewrequest=False)))
...
class DownloadDiffActionTests(ReadOnlyActionTestsMixin, ActionsTestCase):
	...
	def _create_request_context(self, review_request=None, url_name="view-diff", *args, **kwargs):
		"""Create and return objects for use in the request context.
        Args:
		...
		return {"request": self.request, "review_request": review_request}
	def test_get_url_on_diff_viewer(self):
		...
		self.assertEqual(self.action.get_url({"request": self.request}), "raw/")
	...
	def test_get_url_on_review_request(self):
		...
		review_request = self.create_review_request()
		self.assertEqual(self.action.get_url({"request": self.request, "review_request": review_request}), "/r/%s/diff/raw/" % review_request.display_id)
	...
	def test_get_url_on_review_request_with_local_site(self):
		...
		self.assertEqual(self.action.get_url({"request": self.request, "review_request": review_request}), "/s/%s/r/%s/diff/raw/" % (self.local_site_name, review_request.display_id))
	...
	def test_get_hidden_on_review_request(self):
		...
		self.request.resolver_match.url_name = "review-request-detail"
		review_request = self.create_review_request()
		self.assertFalse(self.action.get_hidden({"request": self.request, "review_request": review_request}))
	def test_should_render_on_diff_viewer(self):
		"""Testing DownloadDiffAction.should_render on diff viewer page"""
		self.request.resolver_match = Mock()
		self.request.resolver_match.url_name = "view-diff"
		review_request = self.create_review_request()
		self.assertTrue(self.action.should_render({"request": self.request, "review_request": review_request}))
	...
	def test_should_render_on_review_request_without_repository(self):
		...
		self.request.resolver_match.url_name = "review-request-detail"
		review_request = self.create_review_request()
		self.assertFalse(self.action.should_render({"request": self.request, "review_request": review_request}))
...
class SubmitActionTests(ReadOnlyActionTestsMixin, ActionsTestCase):
	...
	def test_should_render_with_published(self):
		...
		self.assertTrue(self.action.should_render(self._create_request_context(public=True)))
	def test_should_render_with_unpublished(self):
		"""Testing SubmitAction.should_render with unpublished review request
        """
		self.assertFalse(self.action.should_render(self._create_request_context(public=False)))
class UpdateMenuActionTests(ReadOnlyActionTestsMixin, ActionsTestCase):
	...
	fixtures = ["test_users"]
	def _create_request_context(self, public=True, status=ReviewRequest.PENDING_REVIEW, user=None, can_edit_reviewrequest=True, *args, **kwargs):
		"""Create and return objects for use in the request context.
        Args:
            public (bool, optional):
                Whether the review request should be public.
		...
		review_request = self.create_review_request(public=public, status=status)
		self.request.user = user or review_request.submitter
		return {"review_request": review_request, "request": self.request, "perms": {"reviews": {"can_edit_reviewrequest": can_edit_reviewrequest}}}
	def test_should_render_for_owner(self):
		"""Testing UpdateMenuAction.should_render for owner of review request
        """
		self.assertTrue(self.action.should_render(self._create_request_context(can_edit_reviewrequest=False)))
	def test_should_render_for_user(self):
		"""Testing UpdateMenuAction.should_render for normal user"""
		self.assertFalse(self.action.should_render(self._create_request_context(user=User.objects.create_user(username="test-user", email="user@example.com"), can_edit_reviewrequest=False)))
	def test_should_render_user_with_can_edit_reviewrequest(self):
        ...
        """
		self.assertTrue(self.action.should_render(self._create_request_context(user=User.objects.create_user(username="test-user", email="user@example.com"))))
	def test_should_render_with_discarded(self):
		"""Testing UpdateMenuAction.should_render with discarded review request
        """
		self.assertFalse(self.action.should_render(self._create_request_context(status=ReviewRequest.DISCARDED, can_edit_reviewrequest=False)))
	def test_should_render_with_submitted(self):
		"""Testing UpdateMenuAction.should_render with submitted review request
        """
		self.assertFalse(self.action.should_render(self._create_request_context(status=ReviewRequest.SUBMITTED, can_edit_reviewrequest=False)))
class UploadDiffActionTests(ReadOnlyActionTestsMixin, ActionsTestCase):
	...
	def _create_request_context(self, create_repository=True, user=None, *args, **kwargs):
		"""Create and return objects for use in the request context.
        Args:
        ...
        Returns:
            ...
            Additional context to use when testing read-only actions.
        """
		review_request = self.create_review_request(create_repository=create_repository)
		self.request.user = user or review_request.submitter
		return {"review_request": review_request, "request": self.request}
	def test_get_label_with_no_diffs(self):
		...
		self.assertEqual(self.action.get_label({"review_request": review_request, "request": self.request}), "Upload Diff")
	def test_get_label_with_diffs(self):
		"""Testing UploadDiffAction.get_label with diffs"""
		review_request = self.create_review_request(create_repository=True)
		self.create_diffset(review_request)
		self.request.user = review_request.submitter
		self.assertEqual(self.action.get_label({"review_request": review_request, "request": self.request}), "Update Diff")
	...
	def test_should_render_without_repository(self):
		...
		self.assertFalse(self.action.should_render(self._create_request_context(create_repository=False)))