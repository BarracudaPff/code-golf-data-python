...
from reviewboard.reviews.fields import BaseEditableField, BaseTextAreaField, get_review_request_fieldset
from reviewboard.reviews.models import ReviewRequest, ReviewRequestDraft, Screenshot
...
class ReviewRequestDraftTests(TestCase):
	...
	def test_create_with_new_draft(self):
		...
		review_request = self.create_review_request(publish=True, bugs_closed="1,20,300", commit_id="abc123", description_rich_text=True, depends_on=[dep_review_request_1, dep_review_request_2], rich_text=True, target_groups=[group1, group2], target_people=[user1, user2], testing_done_rich_text=True, extra_data={"key": {"values": [1, 2, 3]}, "mybool": True})
		...
		inactive_file_attachment = self.create_file_attachment(review_request, active=False)
		active_screenshot_1 = self.create_screenshot(review_request)
		active_screenshot_2 = self.create_screenshot(review_request)
		inactive_screenshot = self.create_screenshot(review_request, active=False)
		...
		self.assertEqual(draft.description_rich_text, review_request.description_rich_text)
		...
		self.assertEqual(draft.testing_done_rich_text, review_request.testing_done_rich_text)
		self.assertEqual(list(draft.depends_on.order_by("pk")), [dep_review_request_1, dep_review_request_2])
		self.assertEqual(list(draft.target_groups.all()), [group1, group2])
		self.assertEqual(list(draft.target_people.all()), [user1, user2])
		self.assertEqual(list(draft.file_attachments.all()), [active_file_attachment_1, active_file_attachment_2])
		self.assertEqual(list(draft.inactive_file_attachments.all()), [inactive_file_attachment])
		self.assertEqual(list(draft.screenshots.all()), [active_screenshot_1, active_screenshot_2])
		self.assertEqual(list(draft.inactive_screenshots.all()), [inactive_screenshot])
	...
	def test_create_with_new_draft_and_custom_changedesc(self):
        ...
        ChangeDescription
        """
		review_request = self.create_review_request(publish=True, bugs_closed="1,20,300", commit_id="abc123", description_rich_text=True, rich_text=True, testing_done_rich_text=True, extra_data={"key": {"values": [1, 2, 3]}, "mybool": True})
		changedesc = ChangeDescription.objects.create()
		orig_draft = ReviewRequestDraft.create(review_request, changedesc=changedesc)
	...
	def test_create_with_existing_new_draft_new_custom_changedesc(self):
		...
		draft = ReviewRequestDraft.create(review_request, changedesc=new_changedesc)
		self.assertEqual(orig_draft, draft)
		self.assertEqual(draft.changedesc, new_changedesc)
		draft = ReviewRequestDraft.objects.get(pk=draft.pk)
		self.assertEqual(orig_draft, draft)
		self.assertEqual(draft.changedesc, new_changedesc)
		self.assertEqual(list(ChangeDescription.objects.all()), [new_changedesc])
	def test_create_with_existing_new_draft_existing_custom_changedesc(self):
		...
		draft = ReviewRequestDraft.create(review_request, changedesc=orig_changedesc)
		self.assertEqual(orig_draft, draft)
		self.assertEqual(draft.changedesc, orig_changedesc)
		draft = ReviewRequestDraft.objects.get(pk=draft.pk)
		self.assertEqual(orig_draft, draft)
		self.assertEqual(draft.changedesc, orig_changedesc)
		self.assertEqual(list(ChangeDescription.objects.all()), [orig_changedesc])
	...
	def test_publish_with_add_first_file_attachment(self):
		...
		attachment = self.create_file_attachment(review_request, draft=draft, caption="", draft_caption="Test Caption")
		...
		fields = changes.fields_changed
		self.assertEqual(fields["files"], {"new": [(attachment.display_name, attachment.get_absolute_url(), attachment.pk)], "added": [(attachment.display_name, attachment.get_absolute_url(), attachment.pk)], "old": [], "removed": []})
	...
	def test_publish_with_add_another_file_attachment(self):
		...
		attachment1 = self.create_file_attachment(review_request, caption="File 1")
		...
		self.assertEqual(review_request.file_attachments_count, 1)
		self.assertEqual(review_request.inactive_file_attachments_count, 0)
		attachment2 = self.create_file_attachment(review_request, caption="File 2", draft_caption="New File 2", draft=draft)
	...
	def test_publish_with_delete_file_attachment(self):
		...
		user = User.objects.create_user(username="testuser")
		review_request = self.create_review_request(target_people=[user])
		attachment = self.create_file_attachment(review_request, caption="File 1")
		review_request.publish(review_request.submitter)
		draft = ReviewRequestDraft.create(review_request)
		self.assertEqual(draft.file_attachments_count, 1)
		self.assertEqual(draft.inactive_file_attachments_count, 0)
		...
		changes = draft.publish()
		fields = changes.fields_changed
		self.assertEqual(fields["files"], {"new": [], "added": [], "old": [(attachment.display_name, attachment.get_absolute_url(), attachment.pk)], "removed": [(attachment.display_name, attachment.get_absolute_url(), attachment.pk)]})
		self.assertEqual(review_request.file_attachments_count, 0)
	...
	def test_publish_with_add_first_screenshot(self):
		...
		screenshot = self.create_screenshot(review_request, draft=draft, caption="", draft_caption="New Caption")
		...
		fields = changes.fields_changed
		self.assertEqual(fields["screenshots"], {"new": [(screenshot.caption, screenshot.get_absolute_url(), screenshot.pk)], "added": [(screenshot.caption, screenshot.get_absolute_url(), screenshot.pk)], "old": [], "removed": []})
	...
	def test_publish_with_add_another_screenshot(self):
		...
		self.assertEqual(review_request.screenshots_count, 1)
		self.assertEqual(review_request.inactive_screenshots_count, 0)
		screenshot2 = self.create_screenshot(review_request, caption="File 2", draft_caption="New File 2", draft=draft)
		...
		fields = changes.fields_changed
		self.assertEqual(fields["screenshots"], {"new": [(screenshot1.caption, screenshot1.get_absolute_url(), screenshot1.pk), (screenshot2.caption, screenshot2.get_absolute_url(), screenshot2.pk)], "added": [(screenshot2.caption, screenshot2.get_absolute_url(), screenshot2.pk)], "old": [(screenshot1.caption, screenshot1.get_absolute_url(), screenshot1.pk)], "removed": []})
	...
	def test_publish_with_delete_screenshot(self):
		...
		user = User.objects.create_user(username="testuser")
		review_request = self.create_review_request(target_people=[user])
		attachment = self.create_screenshot(review_request, caption="File 1")
		review_request.publish(review_request.submitter)
		draft = ReviewRequestDraft.create(review_request)
		self.assertEqual(draft.screenshots_count, 1)
		self.assertEqual(draft.inactive_screenshots_count, 0)
		...
		changes = draft.publish()
		fields = changes.fields_changed
		self.assertEqual(fields["screenshots"], {"new": [], "added": [], "old": [(attachment.caption, attachment.get_absolute_url(), attachment.pk)], "removed": [(attachment.caption, attachment.get_absolute_url(), attachment.pk)]})
		self.assertEqual(review_request.screenshots_count, 0)
	...
	def test_publish_with_custom_fields(self):
		...
		try:
			...
			draft.target_people.add(review_request.submitter)
			draft.publish()
			self.assertNotIn("description_text_type", review_request.extra_data)
			...
			self.assertNotIn("basic_field_text_type", review_request.extra_data)
			self.assertEqual(review_request.description, draft.description)
			self.assertEqual(review_request.extra_data["rich_field"], draft.extra_data["rich_field"])
			self.assertEqual(review_request.extra_data["rich_field_text_type"], draft.extra_data["rich_field_text_type"])
			self.assertEqual(review_request.extra_data["text"], draft.extra_data["text"])
			self.assertEqual(review_request.extra_data["text_type"], draft.extra_data["text_type"])
			self.assertEqual(review_request.extra_data["basic_field"], draft.extra_data["basic_field"])
		finally:
	...
	def test_publish_without_reviewer_or_group(self):
		...
		draft.branch = "New branch"
		draft.bugs_closed = "12, 34, 56"
		error_message = "There must be at least one reviewer before this " "review request can be published."
	...
	def test_publish_with_history_no_commits_in_diffset(self):
        ...
        """
		review_request = self.create_review_request(create_with_history=True, create_repository=True)
	...
	def test_publish_with_history_diffset_not_finalized(self):
		...
		with override_feature_check(dvcs_feature.feature_id, enabled=True):
			...
			error_msg = "Error publishing: There are no commits attached to the diff"
			with self.assertRaisesMessage(PublishError, error_msg):
				draft.publish()
	def test_publish_with_history_diffset_finalized(self):
		...
		with override_feature_check(dvcs_feature.feature_id, enabled=True):
			review_request = self.create_review_request(create_with_history=True, create_repository=True)
			diffset = self.create_diffset(review_request=review_request, draft=True)
...
class PostCommitTests(SpyAgency, TestCase):
	...
	def setUp(self):
		...
		self.user = User.objects.create_user(username="testuser", password="", email="email@example.com")
		self.profile = self.user.get_profile()
		self.testdata_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "..", "scmtools", "testdata")
	...
	def test_update_from_committed_change(self):
		...
		def get_change(repository, commit_to_get):
			...
			commit = Commit(message="This is my commit message\n\n" "With a summary line too.", diff=self.DEFAULT_GIT_README_DIFF)
			return commit
		review_request = ReviewRequest.objects.create(self.user, self.repository)
		draft = ReviewRequestDraft.create(review_request)
	...
	def test_update_from_committed_change_with_rich_text_reset(self):
        ...
        """
		def get_change(repository, commit_to_get):
			commit = Commit(message="* This is a summary\n\n* This is a description.", diff=self.DEFAULT_GIT_README_DIFF)
			return commit
		review_request = ReviewRequest.objects.create(self.user, self.repository)
		draft = ReviewRequestDraft.create(review_request)
		self.spy_on(draft.repository.get_change, call_fake=get_change)
