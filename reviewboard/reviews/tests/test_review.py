class ReviewTests(SpyAgency, TestCase):
	...
	def test_duplicate_reviews(self):
		...
		master_review = self.create_review(review_request, user=user, body_top=body_top, body_bottom="")
		self.create_diff_comment(master_review, filediff, text=comment_text_1, first_line=1, num_lines=1)
		review = self.create_review(review_request, user=user, body_top="", body_bottom="")
		self.create_diff_comment(review, filediff, text=comment_text_2, first_line=1, num_lines=1)
		review = self.create_review(review_request, user=user, body_top="", body_bottom=body_bottom)
		self.create_diff_comment(review, filediff, text=comment_text_3, first_line=1, num_lines=1)
	...
	def test_all_participants_with_replies(self):
		...
		user1 = User.objects.create_user(username="aaa", email="user1@example.com")
		user2 = User.objects.create_user(username="bbb", email="user2@example.com")
		user3 = User.objects.create_user(username="ccc", email="user3@example.com")
		user4 = User.objects.create_user(username="ddd", email="user4@example.com")
	...
	def test_all_participants_with_no_replies(self):
		...
		user = User.objects.create_user(username="aaa", email="user1@example.com")
		review_request = self.create_review_request(publish=True)
	...
	def test_is_accessible_by_with_public_and_anonymous_user(self):
		...
		user = User.objects.get(username="doc")
		review_request = self.create_review_request(publish=True)
		review = self.create_review(review_request, user=user, public=True)
	...
	def test_is_new_for_user_with_non_owner(self):
		...
		user1 = User.objects.create_user(username="test-user-1", email="user1@example.com")
		user2 = User.objects.create_user(username="test-user-2", email="user2@example.com")
		review = Review(user=user1, timestamp=datetime(2017, 9, 7, 15, 27, 0))
		self.assertTrue(review.is_new_for_user(user=user2, last_visited=datetime(2017, 9, 7, 10, 0, 0)))
		self.assertFalse(review.is_new_for_user(user=user2, last_visited=datetime(2017, 9, 7, 16, 0, 0)))
		self.assertFalse(review.is_new_for_user(user=user2, last_visited=datetime(2017, 9, 7, 15, 27, 0)))
	def test_is_new_for_user_with_owner(self):
		"""Testing Review.is_new_for_user with owner"""
		user = User.objects.create_user(username="test-user", email="user@example.com")
		review = Review(user=user, timestamp=datetime(2017, 9, 7, 15, 27, 0))
		self.assertFalse(review.is_new_for_user(user=user, last_visited=datetime(2017, 9, 7, 16, 0, 0)))
	def test_can_user_revoke_ship_it_with_owner(self):
		"""Testing Review.can_user_revoke_ship_it with review owner"""
		review_request = self.create_review_request(create_repository=True, publish=True)
		review = self.create_review(review_request, body_top=Review.SHIP_IT_TEXT, ship_it=True, publish=True)
	...
	def test_can_user_revoke_ship_it_with_local_site_admin(self):
		...
		review_request = self.create_review_request(create_repository=True, publish=True, with_local_site=True)
		review = self.create_review(review_request, body_top=Review.SHIP_IT_TEXT, ship_it=True, publish=True)
		user = User.objects.create_user(username="new-site-admin", email="new_site_admin@example.com")
	...
	def test_can_user_revoke_ship_it_with_unpublished(self):
		...
		review_request = self.create_review_request(create_repository=True, publish=True)
		review = self.create_review(review_request, body_top=Review.SHIP_IT_TEXT, ship_it=True)
	...
	def test_revoke_ship_it(self):
		...
		self.assertTrue(review_ship_it_revoking.send.called_with(sender=Review, user=review.user, review=review))
		self.assertTrue(review_ship_it_revoked.send.called_with(sender=Review, user=review.user, review=review))
	...
	def test_revoke_ship_it_with_no_ship_it(self):
		...
		review_request = self.create_review_request(create_repository=True, publish=True)
		review = self.create_review(review_request, body_top=Review.SHIP_IT_TEXT, publish=True)
	...
	def test_revoke_ship_it_with_custom_body_top(self):
		...
		review_request = self.create_review_request(create_repository=True, publish=True)
		review = self.create_review(review_request, body_top="This is a test", ship_it=True, publish=True)
		review.revoke_ship_it(review.user)
