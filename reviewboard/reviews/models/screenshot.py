...
class Screenshot(models.Model):
	...
	draft_caption = models.CharField(_("draft caption"), max_length=256, blank=True)
	image = models.ImageField(_("image"), upload_to=os.path.join("uploaded", "images", "%Y", "%m", "%d"))
	@property
	def filename(self):
	...
	def thumb(self):
		...
		return format_html('<img src="{src_1x}" srcset="{src_1x} 1x, {src_2x} 2x"' ' alt="{caption}" width="400" />', src_1x=url, src_2x=thumbnail(self.image, "800x200"), caption=self.caption)
	...
	def get_absolute_url(self):
		...
		return local_site_reverse("screenshot", local_site_name=local_site_name, kwargs={"review_request_id": review_request.display_id, "screenshot_id": self.pk})
