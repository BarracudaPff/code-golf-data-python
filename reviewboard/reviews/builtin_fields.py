...
class BaseCaptionsField(ReviewRequestPageDataMixin, BaseReviewRequestField):
	...
	def render_change_entry_html(self, info):
		...
		for id_str, caption in six.iteritems(info):
			...
			s.append(format_html("<tr>" ' <th><a href="{url}">{filename}</a>:</th>' " <td>{caption}</td>" "</tr>", url=obj.get_absolute_url(), filename=obj.filename, caption=mark_safe(render_item(caption))))
		s.append("</table>")
		return "".join(s)
	def serialize_change_entry(self, changedesc):
		...
		return [{"old": data[six.text_type(obj.pk)]["old"][0], "new": data[six.text_type(obj.pk)]["new"][0], self.caption_object_field: obj} for obj in self.model.objects.filter(pk__in=six.iterkeys(data))]
...
class OwnerField(BuiltinFieldMixin, BaseEditableField):
	...
	def render_value(self, user):
        ...
        Returns:
            ...
            The rendered value.
        """
		return format_html('<a class="user" href="{0}">{1}</a>', local_site_reverse("user", local_site=self.review_request_details.local_site, args=[user]), user.get_profile().get_display_name(self.request.user))
	def record_change_entry(self, changedesc, old_value, new_value):
		"""Record information on the changed values in a ChangeDescription.
        Args:
            changedesc (reviewboard.changedescs.models.ChangeDescription):
...
class BugsField(BuiltinFieldMixin, BaseCommaEditableField):
	...
	def render_item(self, bug_id):
		...
		if bug_url:
			return format_html('<a class="bug" href="{url}">{id}</a>', url=bug_url, id=bug_id)
		else:
	...
	def _get_bug_url(self, bug_id):
		...
		try:
			if repository and repository.bug_tracker and "%s" in repository.bug_tracker:
				bug_url = local_site_reverse("bug_url", local_site_name=local_site_name, args=[review_request.display_id, bug_id])
...
class DependsOnField(BuiltinFieldMixin, BaseModelListEditableField):
	...
	def render_change_entry_item_html(self, info, item):
		...
		rendered_item = format_html('<a href="{url}">{id} - {summary}</a>', url=item.get_absolute_url(), id=item.pk, summary=item.summary)
	...
	def render_item(self, item):
        ...
        Returns:
            ...
            The rendered item.
        """
		rendered_item = format_html('<a href="{url}" title="{summary}"' '   class="review-request-link">{id}</a>', url=item.get_absolute_url(), summary=item.summary, id=item.display_id)
		if item.status in (ReviewRequest.SUBMITTED, ReviewRequest.DISCARDED):
			return "<s>%s</s>" % rendered_item
		else:
			return rendered_item
class BlocksField(BuiltinFieldMixin, BaseReviewRequestField):
	...
	def render_value(self, blocks):
        ...
        Returns:
            ...
            The rendered value.
        """
		return format_html_join(", ", '<a href="{0}" class="review-request-link">{1}</a>', [(item.get_absolute_url(), item.display_id) for item in blocks])
...
class CommitField(BuiltinFieldMixin, BaseReviewRequestField):
	...
	def render_value(self, commit_id):
		...
		if len(commit_id) == 40:
			...
			return '<span title="%s">%s</span>' % (escape(commit_id), escape(abbrev_commit_id))
		else:
...
class DiffField(ReviewRequestPageDataMixin, BuiltinFieldMixin, BaseReviewRequestField):
	...
	def render_change_entry_html(self, info):
		...
		if raw_insert_count > 0:
			line_counts.append('<span class="insert-count">+%d</span>' % raw_insert_count)
		if raw_delete_count > 0:
			line_counts.append('<span class="delete-count">-%d</span>' % raw_delete_count)
		s.append(format_html('<p class="diff-changes">' ' <a href="{url}">{label}</a>' ' <span class="line-counts">({line_counts})</span>' "</p>", url=diff_url, label=_("Revision %s") % diff_revision, count=_("%d files") % len(diffset.cumulative_files), line_counts=mark_safe(" ".join(line_counts))))
		if past_revision > 0:
			interdiff_url = local_site_reverse("view-interdiff", local_site=review_request.local_site, args=[review_request.display_id, past_revision, diff_revision])
			s.append(format_html('<p><a href="{url}">{text}</a>', url=interdiff_url, text=_("Show changes")))
		file_count = len(diffset.cumulative_files)
		if file_count > 0:
			s += ['<div class="diff-index">', " <table>"]
			files = get_sorted_filediffs(enumerate(diffset.cumulative_files), key=lambda i: i[1])
			for i, filediff in files[: self.MAX_FILES_PREVIEW]:
				counts = filediff.get_line_counts()
				data_attrs = ['data-%s="%s"' % (attr.replace("_", "-"), counts[attr]) for attr in ("insert_count", "delete_count", "replace_count", "total_line_count") if counts.get(attr) is not None]
				s.append(format_html("<tr {data_attrs}>" ' <td class="diff-file-icon"></td>' ' <td class="diff-file-info">' '  <a href="{url}">{filename}</a>' " </td>" "</tr>", data_attrs=mark_safe(" ".join(data_attrs)), url=diff_url + "#%d" % i, filename=filediff.source_file))
			num_remaining = file_count - self.MAX_FILES_PREVIEW
			if num_remaining > 0:
				s.append(format_html("<tr>" " <td></td>" ' <td class="diff-file-info">{text}</td>' "</tr>", text=_("%s more") % num_remaining))
			s += [" </table>", "</div>"]
		return "".join(s)
	def has_value_changed(self, old_value, new_value):
		"""Return whether the value has changed.
	...
	def record_change_entry(self, changedesc, unused, diffset):
        ...
        """
		review_request = self.review_request_details.get_review_request()
		url = local_site_reverse("view-diff-revision", local_site=review_request.local_site, args=[review_request.display_id, diffset.revision])
		changedesc.fields_changed["diff"] = {"added": [(_("Diff r%s") % diffset.revision, url, diffset.pk)]}
	def serialize_change_entry(self, changedesc):
		"""Serialize a change entry for public consumption.
        This will output a version of the change entry for use in the API.
        It can be the same content stored in the
		...
		return {"added": DiffSet.objects.get(pk=diffset_id)}
...
class FileAttachmentsField(ReviewRequestPageDataMixin, BuiltinFieldMixin, BaseCommaEditableField):
	...
	def get_change_entry_sections_html(self, info):
		...
		sections = []
		if "removed" in info:
			sections.append({"title": _("Removed Files"), "rendered_html": mark_safe(self.render_change_entry_html(info["removed"]))})
		if "added" in info:
			sections.append({"title": _("Added Files"), "rendered_html": mark_safe(self.render_change_entry_html(info["added"]))})
		return sections
		...
		for caption, filename, pk in values:
			if pk in self.data.file_attachments_by_id:
				...
				try:
					...
					continue
			items.append(template.render(Context({"file": attachment, "review_request": review_request, "local_site_name": local_site_name, "request": self.request, "uuid": uuid.uuid4()})))
		return "".join(items)
...
class TargetGroupsField(BuiltinFieldMixin, BaseModelListEditableField):
	...
	def render_item(self, group):
        ...
        Returns:
            ...
            The rendered item.
        """
		return '<a href="%s">%s</a>' % (escape(group.get_absolute_url()), escape(group.name))
class TargetPeopleField(BuiltinFieldMixin, BaseModelListEditableField):
	...
	def render_item(self, user):
		...
		return format_html('<a href="{0}" class="{1}">{2}</a>', local_site_reverse("user", local_site=self.review_request_details.local_site, args=[user]), " ".join(extra_classes), user.username)
class CommitListField(ReviewRequestPageDataMixin, BaseReviewRequestField):
	...
	def review_request_created_with_history(self):
		...
		return self.review_request_details.get_review_request().created_with_history
	@property
	def should_render(self):
		...
		return self.review_request_created_with_history and url_name not in diffviewer_url_names
	@property
	...
	def render_value(self, value):
		...
		if not value:
			return ""
		commits = list(DiffCommit.objects.filter(diffset_id=value).order_by("id"))
		context = self._get_common_context(commits)
		context["commits"] = commits
		return render_to_string(template_name="reviews/commit_list_field.html", request=self.request, context=context)
	def has_value_changed(self, old_value, new_value):
	...
	def record_change_entry(self, changedesc, old_value, new_value):
        ...
        """
		changedesc.fields_changed[self.field_id] = {"old": old_value.pk, "new": new_value.pk}
	def render_change_entry_html(self, info):
		...
		context.update({"old_commits": old_commits, "new_commits": new_commits})
		return render_to_string(template_name="reviews/changedesc_commit_list.html", request=self.request, context=context)
	def serialize_change_entry(self, changedesc):
		...
		commits_by_diffset_id = DiffCommit.objects.by_diffset_ids((info["old"], info["new"]))
		return {key: [{"author": commit.author_name, "summary": commit.summary} for commit in commits_by_diffset_id[info[key]]] for key in ("old", "new")}
	def _get_common_context(self, commits):
		...
		return {"include_author_name": include_author_name, "to_expand": to_expand}
class MainFieldSet(BaseReviewRequestFieldSet):
	fieldset_id = "main"
	field_classes = [SummaryField, DescriptionField, TestingDoneField]
class ExtraFieldSet(BaseReviewRequestFieldSet):
	"""A field set that is displayed after the main field set."""
	fieldset_id = "extra"
	field_classes = [CommitListField]
class InformationFieldSet(BaseReviewRequestFieldSet):
	fieldset_id = "info"
	label = _("Information")
	field_classes = [OwnerField, RepositoryField, BranchField, BugsField, DependsOnField, BlocksField, ChangeField, CommitField]
class ReviewersFieldSet(BaseReviewRequestFieldSet):
	fieldset_id = "reviewers"
	label = _("Reviewers")
	show_required = True
	field_classes = [TargetGroupsField, TargetPeopleField]
class ChangeEntryOnlyFieldSet(BaseReviewRequestFieldSet):
	fieldset_id = "_change_entries_only"
	field_classes = [DiffField, FileAttachmentCaptionsField, ScreenshotCaptionsField, FileAttachmentsField, ScreenshotsField, StatusField]
builtin_fieldsets = [MainFieldSet, ExtraFieldSet, InformationFieldSet, ReviewersFieldSet, ChangeEntryOnlyFieldSet]