...
class AnyReviewGroupsPublicOperator(BaseConditionOperator):
	...
	def matches(self, match_value, **kwargs):
        ...
        """
		return match_value and any(not group.invite_only for group in match_value)
class AllReviewGroupsInviteOnlyOperator(BaseConditionOperator):
	...
	def matches(self, match_value, **kwargs):
        ...
        """
		return match_value and all(group.invite_only for group in match_value)
class ReviewGroupsChoice(BaseConditionModelMultipleChoice):
	...
	operators = ConditionOperators([AnyOperator.with_overrides(name=_("Any review groups")), UnsetOperator.with_overrides(name=_("No review groups")), ContainsAnyOperator, DoesNotContainAnyOperator, AnyReviewGroupsPublicOperator, AllReviewGroupsInviteOnlyOperator])
	def get_queryset(self):
		...
		if self.extra_state.get("matching"):
			...
			return Group.objects.accessible(user=request.user, local_site=local_site, show_all_local_sites=show_all_local_sites)
	def get_match_value(self, review_groups, value_state_cache, **kwargs):
		...
		return result
class ReviewRequestBranchChoice(ReviewRequestConditionChoiceMixin, BaseConditionStringChoice):
...
class ReviewRequestDescriptionChoice(ReviewRequestConditionChoiceMixin, BaseConditionStringChoice):
...
class BaseReviewRequestDiffFileChoice(ReviewRequestConditionChoiceMixin, ConditionChoiceMatchListItemsMixin, BaseConditionStringChoice):
	...
	def get_match_value(self, review_request, value_state_cache, **kwargs):
		...
		try:
			...
			result = set(chain.from_iterable(diffset.files.values_list("source_file", "dest_file")))
...
class ReviewRequestOwnerChoice(LocalSiteModelChoiceMixin, ReviewRequestConditionChoiceMixin, BaseConditionModelMultipleChoice):
	"""A condition choice for matching a review request's owner."""
	queryset = User.objects.all()
	choice_id = "owner"
	name = _("Owner")
	operators = ConditionOperators([IsOneOfOperator, IsNotOneOfOperator])
	def get_match_value(self, review_request, **kwargs):
...
class ReviewRequestReviewerChoice(LocalSiteModelChoiceMixin, ReviewRequestConditionChoiceMixin, BaseConditionModelMultipleChoice):
	"""A condition choice for matching a review request's reviewer."""
	queryset = User.objects.all()
	choice_id = "reviewer"
	name = _("Reviewer")
	operators = ConditionOperators([ContainsAnyOperator, DoesNotContainAnyOperator])
	def get_match_value(self, review_request, **kwargs):
...
class ReviewRequestParticipantChoice(LocalSiteModelChoiceMixin, ReviewRequestConditionChoiceMixin, BaseConditionModelMultipleChoice):
...
class ReviewRequestSummaryChoice(ReviewRequestConditionChoiceMixin, BaseConditionStringChoice):
...
class ReviewRequestRepositoriesChoice(ReviewRequestConditionChoiceMixin, RepositoriesChoice):
...
class ReviewRequestRepositoryTypeChoice(ReviewRequestConditionChoiceMixin, RepositoryTypeChoice):
	...
	def get_match_value(self, review_request, **kwargs):
        ...
        Returns:
            ...
            The review request's repository.
        """
		return super(ReviewRequestRepositoryTypeChoice, self).get_match_value(repository=review_request.repository)
class ReviewRequestReviewGroupsChoice(ReviewRequestConditionChoiceMixin, ReviewGroupsChoice):
	...
	def get_match_value(self, review_request, **kwargs):
        ...
        """
		return super(ReviewRequestReviewGroupsChoice, self).get_match_value(review_groups=review_request.target_groups, **kwargs)
class ReviewRequestConditionChoices(ConditionChoices):
    ...
    """
	choice_classes = [ReviewRequestBranchChoice, ReviewRequestDescriptionChoice, ReviewRequestRepositoriesChoice, ReviewRequestRepositoryTypeChoice, ReviewRequestReviewGroupsChoice, ReviewRequestOwnerChoice, ReviewRequestReviewerChoice, ReviewRequestParticipantChoice, ReviewRequestSummaryChoice, ReviewRequestTestingDoneChoice]