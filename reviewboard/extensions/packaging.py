...
class BuildStaticFiles(DjbletsBuildStaticFiles):
	...
	def get_lessc_global_vars(self):
		...
		global_vars.update({"RB_MAJOR_VERSION": VERSION[0], "RB_MINOR_VERSION": VERSION[1], "RB_MICRO_VERSION": VERSION[2], "RB_PATCH_VERSION": VERSION[3], "RB_IS_RELEASED": VERSION[5]})
		return global_vars
def setup(**setup_kwargs):
	...
	setup_kwargs.update({"zip_safe": False, "include_package_data": True, "cmdclass": dict(build_extension_cmdclass(BuildStaticFiles), **setup_kwargs.get("cmdclass", {}))})
