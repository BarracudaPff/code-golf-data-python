...
class Client(base.Client):
	...
	def __init__(self, config_dir, repopath, username=None, password=None):
		...
		if repopath.startswith("https:"):
			auth_providers += [ra.get_ssl_client_cert_file_provider(), ra.get_ssl_client_cert_pw_file_provider(), ra.get_ssl_server_trust_file_provider(), ra.get_ssl_server_trust_prompt_provider(self.ssl_trust_prompt)]
		self.auth = ra.Auth(auth_providers)
		self.username = None
		self.password = None
		if username:
			self.username = username
			self.auth.set_parameter(AUTH_PARAM_DEFAULT_USERNAME, self.username)
		if password:
			self.password = password
			self.auth.set_parameter(AUTH_PARAM_DEFAULT_PASSWORD, self.password)
	...
	def get_keywords(self, path, revision=HEAD):
		...
		revnum = self._normalize_revision(revision)
		path = self.normalize_path(path)
		return self.client.propget("svn:keywords", path, None, revnum).get(path)
	...
	def repository_info(self):
		...
		return {"uuid": force_text(info.repos_uuid), "root_url": force_text(info.repos_root_url), "url": force_text(info.url)}
	def ssl_trust_prompt(self, realm, failures, certinfo, may_save):
		...
		if self._ssl_trust_prompt_cb:
			trust_dict = {"realm": realm, "failures": failures, "hostname": certinfo[0], "finger_print": certinfo[1], "valid_from": certinfo[2], "valid_until": certinfo[3], "issuer_dname": certinfo[4]}
	...
	def accept_ssl_certificate(self, path, on_failure=None):
		...
		def _accept_trust_prompt(realm, failures, certinfo, may_save):
			cert.update({"realm": realm, "failures": failures, "hostname": certinfo[0], "finger_print": certinfo[1], "valid_from": certinfo[2], "valid_until": certinfo[3], "issuer_dname": certinfo[4]})
			if on_failure:
				return 0, False
			else:
				del cert["failures"]
				return failures, True
		auth = ra.Auth([ra.get_simple_provider(), ra.get_username_provider(), ra.get_ssl_client_cert_file_provider(), ra.get_ssl_client_cert_pw_file_provider(), ra.get_ssl_server_trust_file_provider(), ra.get_ssl_server_trust_prompt_provider(_accept_trust_prompt)])
		if self.username:
	...
	def get_log(self, path, start=None, end=None, limit=None, discover_changed_paths=False, limit_to_path=False):
		...
		def log_cb(changed_paths, revision, props, has_children):
			commit = {"revision": force_text(revision)}
			if "svn:date" in props:
				commit["date"] = datetime.strptime(props["svn:date"].decode("utf-8"), "%Y-%m-%dT%H:%M:%S.%fZ")
		...
		commits = []
		self.client.log(log_cb, paths=self.normalize_path(path), start_rev=self._normalize_revision(start), end_rev=self._normalize_revision(end), limit=limit, discover_changed_paths=discover_changed_paths, strict_node_history=limit_to_path)
		return commits
	def list_dir(self, path):
        ...
        * ``created_rev`` - The revision where the file or directory was
		...
		for name, dirent in six.iteritems(dirents):
			if name:
				result[six.text_type(name)] = {"path": "%s/%s" % (path.strip("/"), name), "created_rev": six.text_type(dirent["created_rev"])}
		return result
	def diff(self, revision1, revision2):
		...
		err = None
		try:
			out, err = self.client.diff(self._normalize_revision(revision1), self._normalize_revision(revision2), self.repopath, self.repopath, diffopts=["-u"])
			diff = out.read()
		except Exception as e:
			logging.error("Failed to generate diff using subvertpy for " "revisions %s:%s for path %s: %s", revision1, revision2, path, e, exc_info=1)
			raise SCMError(_("Unable to get diff revisions %s through %s: %s") % (revision1, revision2, e))
		finally:
			if out:
				out.close()
			if err:
