...
from reviewboard.notifications.email.message import prepare_password_changed_mail
from reviewboard.oauth.features import oauth2_service_feature
from reviewboard.oauth.forms import UserApplicationChangeForm, UserApplicationCreationForm
from reviewboard.oauth.models import Application
from reviewboard.site.mixins import CheckLocalSiteAccessViewMixin
from reviewboard.site.urlresolvers import local_site_reverse
class UserInfoboxView(CheckLoginRequiredViewMixin, CheckLocalSiteAccessViewMixin, ETagViewMixin, TemplateView):
	...
	def get_etag_data(self, request, username, *args, **kwargs):
		...
		etag_data = [user.first_name, user.last_name, user.email, six.text_type(user.last_login), six.text_type(settings.TEMPLATE_SERIAL), six.text_type(self._show_profile), self._timezone]
		...
		for hook in UserInfoboxHook.hooks:
			try:
				etag_data.append(hook.get_etag_data(user=user, request=request, local_site=local_site))
			except Exception as e:
				logging.exception("Error when running UserInfoboxHook." 'get_etag_data method in extension "%s": %s', hook.extension.id, e)
	...
	def get_context_data(self, **kwargs):
		...
		for hook in UserInfoboxHook.hooks:
			try:
				extra_content.append(hook.render(user=user, request=self.request, local_site=local_site))
			except Exception as e:
				logging.exception("Error when running UserInfoboxHook." 'render method in extension "%s": %s', hook.extension.id, e)
		review_requests_url = local_site_reverse("user", local_site=local_site, args=[username])
		reviews_url = local_site_reverse("user-grid", local_site=local_site, args=[username, "reviews"])
		has_avatar = avatar_services.avatars_enabled and avatar_services.for_user(user) is not None
		return {"extra_content": mark_safe("".join(extra_content)), "full_name": user.get_full_name(), "has_avatar": has_avatar, "infobox_user": user, "review_requests_url": review_requests_url, "reviews_url": reviews_url, "show_profile": self._show_profile, "timezone": self._timezone}
@csrf_protect
...
class MyAccountView(ConfigPagesView):
    ...
    """
	title = _("My Account")
	css_bundle_names = ["account-page"]
	js_bundle_names = ["3rdparty-jsonlint", "config-forms", "account-page"]
...
def edit_oauth_app(request, app_id=None):
	...
	if request.method == "POST":
		...
		form = form_cls(user=request.user, data=form_data, initial=None, instance=app)
		if form.is_valid():
			app = form.save()
		...
		form = form_cls(user=request.user, data=None, initial=None, instance=app)
		if app and app.is_disabled_for_security:
			form._errors = ErrorDict({"__all__": form.error_class([form.DISABLED_FOR_SECURITY_ERROR])})
	return render(request=request, template_name="accounts/edit_oauth_app.html", context={"app": app, "form": form, "fieldsets": filter_fieldsets(form=form_cls, fieldsets=fieldsets), "oauth2_page_url": OAuth2Page.get_absolute_url(), "request": request})