class UnfuddleForm(HostingServiceForm):
	unfuddle_account_domain = forms.CharField(label=_("Unfuddle account domain"), max_length=64, required=True, widget=forms.TextInput(attrs={"size": "60"}), help_text=_("This is the <tt>domain</tt> part of " "<tt>domain.unfuddle.com</tt>"))
	unfuddle_project_id = forms.CharField(label=_("Unfuddle project ID"), max_length=64, required=True, widget=forms.TextInput(attrs={"size": "5"}), initial=1, help_text=_("This is the numeric project ID found in the URL " "on your Project page (for example, " "https://myaccount.unfuddle.com/a#/projects/" "<b>&lt;id&gt;</b>)"))
	unfuddle_repo_name = forms.CharField(label=_("Repository name"), max_length=64, required=True, widget=forms.TextInput(attrs={"size": "60"}))
class Unfuddle(HostingService):
	...
	bug_tracker_field = "https://%(unfuddle_account_domain)s.unfuddle.com/a#/projects/" "%(unfuddle_project_id)s/tickets/by_number/%%s"
	form = UnfuddleForm
	repository_fields = {"Git": {"path": "git@%(unfuddle_account_domain)s.unfuddle.com:" "%(unfuddle_account_domain)s/%(unfuddle_repo_name)s.git", "mirror_path": "https://%(unfuddle_account_domain)s.unfuddle.com" "/git/%(unfuddle_account_domain)s_" "%(unfuddle_repo_name)s/"}, "Subversion": {"path": "https://%(unfuddle_account_domain)s.unfuddle.com/svn/" "%(unfuddle_account_domain)s_%(unfuddle_repo_name)s", "mirror_path": "http://%(unfuddle_account_domain)s.unfuddle.com" "/svn/%(unfuddle_account_domain)s_" "%(unfuddle_repo_name)s"}}
	TOOL_NAME_MAP = {"git": "Git", "svn": "Subversion"}
	...
	def get_file(self, repository, path, revision, base_commit_id=None, *args, **kwargs):
		...
		try:
			...
			url = self._build_api_url(self._get_repository_account_domain(repository), "repositories/%s/download/?path=%s&commit=%s" % (self._get_repository_id(repository), quote(path), quote(commit_id)))
	...
	def get_file_exists(self, repository, path, revision, base_commit_id=None, *args, **kwargs):
		...
		try:
			commit_id = self._get_commit_id(repository, path, revision, base_commit_id)
			url = self._build_api_url(self._get_repository_account_domain(repository), "repositories/%s/history/?path=%s&commit=%s&count=0" % (self._get_repository_id(repository), quote(path), quote(commit_id)))
			self._api_get(url)
			return True
	...
	def _get_commit_id(self, repository, path, revision, base_commit_id):
		if base_commit_id:
			...
			raise FileNotFoundError(path, revision, detail="The necessary revision information needed to find " "this file was not provided. Use RBTools 0.5.2 or " "newer.")
		return revision
		...
		for repo in results:
			if repo["abbreviation"] == repository_name:
				...
				if unfuddle_tool_name == tool_name:
					return repo
		raise RepositoryError(ugettext("A repository with this name was not found"))
	...
	def _api_get(self, url, raw_content=False, username=None, password=None):
		try:
			response = self.client.http_get(url, username=username or self.account.username, password=password or self.get_password(), headers={"Accept": "application/json"})
			if raw_content:
			...
			if e.code == 401:
				raise AuthorizationError(ugettext("The login or password is incorrect."))
			raise