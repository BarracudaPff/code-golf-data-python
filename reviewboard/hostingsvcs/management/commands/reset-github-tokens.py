...
class Command(BaseCommand):
	...
	def add_arguments(self, parser):
		...
		parser.add_argument("--local-sites", action="store", dest="local_sites", help=_("Comma-separated list of Local Sites to filter by"))
		...
		for account in accounts:
			if force_yes:
				...
				if account.local_site:
					reset_msg = _("Reset token for %(site_name)s " "(%(username)s) [Y/n] ") % {"site_name": account.local_site.name, "username": account.username}
				else:
					reset_msg = _("Reset token for %s [Y/n] ") % (account.username)
	...
	def _reset_token(self, account):
		...
		while True:
			if not password and service.get_reset_auth_token_requires_password():
				password = getpass.getpass(_("Password for %s: ") % account.username)
				auth_token = None
			try:
