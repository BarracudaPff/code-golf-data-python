...
class GerritAuthForm(HostingServiceAuthForm):
	...
	def save(self, **kwargs):
		...
		hosting_account.data["gerrit_http_password"] = encrypt_password(self.get_credentials()["password"])
		hosting_account.save()
		return hosting_account
	class Meta(object):
		help_texts = {"hosting_account_username": _("Your HTTP username. You can find this under " "<tt>Settings &gt; HTTP Password</tt>."), "hosting_account_password": _("Your HTTP password. You can find this under " "<tt>Settings &gt; HTTP Password</tt>.")}
class GerritForm(HostingServiceForm):
	"""The Gerrit hosting service form."""
	gerrit_url = forms.URLField(label=_("Gerrit URL"), required=True, help_text=_("The full URL of your Gerrit instance, as in " "<tt>http://gerrit.example.com/</tt>."), widget=forms.TextInput(attrs={"size": 60}))
	gerrit_ssh_port = forms.IntegerField(label=_("SSH port"), required=True, help_text=_("The port configured for SSH access to Gerrit."), initial=29418, min_value=1, max_value=65535, widget=forms.TextInput(attrs={"size": 60}))
	gerrit_project_name = forms.CharField(label=_("Project name"), required=True, help_text=_("The name of the project on Gerrit."), widget=forms.TextInput(attrs={"size": 60}))
	def clean(self):
		"""Clean the form.
...
class GerritClient(HostingServiceClient):
	...
	def api_get(self, url, username=None, password=None, json=True, *args, **kwargs):
		...
		if username is None or password is None:
			...
			password = decrypt_password(self.account.data["gerrit_http_password"])
...
class Gerrit(HostingService):
	...
	repository_fields = {"Git": {"path": "ssh://%(hosting_account_username)s@%(gerrit_domain)s:" "%(gerrit_ssh_port)s/%(gerrit_project_name)s", "mirror_path": "ssh://%(hosting_account_username)s@%(gerrit_domain)s/" "%(gerrit_ssh_port)s/%(gerrit_project_name)s"}}
	def check_repository(self, gerrit_url=None, gerrit_project_name=None, *args, **kwargs):
        ...
        """
		url = urljoin(gerrit_url, "a/projects/%s" % quote_plus(gerrit_project_name))
		try:
			self.client.api_get(url, json=False)
		except HostingServiceAPIError as e:
			if e.http_code == 404:
				raise RepositoryError(ugettext('The project "%s" does not exist or you do not ' "have access to it.") % gerrit_project_name)
			raise
		url = urljoin(gerrit_url, "a/plugins/")
		try:
			rsp = self.client.api_get(url)
		except HostingServiceError as e:
			logger.exception("Could not retrieve the list of Gerrit plugins from %s: %s", url, e)
			raise RepositoryError(ugettext("Could not retrieve the list of Gerrit plugins " "from %(url).") % {"url": url})
		if "gerrit-reviewboard" not in rsp:
			raise RepositoryError(ugettext('The "gerrit-reviewboard" plugin is not installed ' "on the server. See %(plugin_url)s for installation " "instructions.") % {"plugin_url": _PLUGIN_URL})
		else:
			version_str = rsp["gerrit-reviewboard"]["version"]
			try:
				version = self._parse_plugin_version(version_str)
			except Exception as e:
				logger.exception('Could not parse gerrit-reviewboard plugin version "%s" ' "from %s: %s", version, url, e)
				raise RepositoryError(ugettext("Could not parse gerrit-reviewboard version: " '"%(version)s" from URL %(url)s: %(error)s') % {"version": version, "error": e, "url": url})
			if version < self.REQUIRED_PLUGIN_VERSION:
				raise RepositoryError(ugettext('The "gerrit-reviewboard" plugin on the server ' "is an incompatible version: found %(found)s but " "version %(required)s or higher is required.") % {"found": version_str, "required": self.REQUIRED_PLUGIN_VERSION_STR})
	def authorize(self, username, password, credentials, local_site_name=None, gerrit_url=None, *args, **kwargs):
		...
		try:
			...
			if e.http_code != 401:
				logger.error("Unknown HTTP response while authenticating with Gerrit " "at %s: %s", url, e)
				raise AuthorizationError(ugettext("Could not authenticate with Gerrit at %(url): " "%(error)s") % {"url": url, "error": e.message}, http_code=e.http_code)
			raise AuthorizationError(ugettext("Unable to authenticate to Gerrit at %(url)s. The " "username or password used may be invalid.") % {"url": url}, http_code=e.http_code)
	...
	def get_branches(self, repository):
		...
		for branch in rsp:
			...
			if ref == "refs/meta/config":
				...
				continue
			branches.append(Branch(id=branch_id, commit=commit, default=(branch_id == "master")))
		return branches
	def get_commits(self, repository, branch=None, start=None, limit=None):
		...
		if not start:
			start = branch
		query = {field: six.text_type(value).encode("utf-8") for field, value in (("start", start), ("limit", limit)) if value is not None}
		url = self._build_project_api_url(repository, ("all-commits",), query=query)
		try:
			rsp = self.client.api_get(url)
		except HostingServiceAPIError as e:
			if limit == 1:
				raise HostingServiceAPIError(ugettext('Could not retrieve commit "%(rev)s": %(error)s') % {"rev": start, "error": e.message}, http_code=e.http_code, rsp=e.rsp)
			else:
				raise HostingServiceAPIError(ugettext("Could not retrieve commits starting at " '"%(rev)s": %(error)s') % {"rev": start, "error": e.message}, http_code=e.getcode(), rsp=e.rsp)
	...
	def get_file(self, repository, path, revision, *args, **kwargs):
        ...
        Raises:
            ...
            reviewboard.hostingsvcs.errors.HostingServiceAPIError:
                An error occurred communicating with the Gerrit API.
        """
		url = self._build_project_api_url(repository, ("blobs", revision, "content"))
		try:
			rsp = self.client.api_get(url, json=False)
		except HostingServiceAPIError as e:
			if e.http_code == 404:
				raise FileNotFoundError(path, revision=revision)
			raise HostingServiceAPIError(ugettext('Could not get file "%(file)s" at revision ' '"%(rev)s": %(error)s') % {"file": path, "rev": revision, "error": e.read()}, http_code=e.http_code)
		try:
			return base64.b64decode(rsp)
		except Exception as e:
			raise HostingServiceAPIError(ugettext('An error occurred while retrieving "%(file)s" at ' 'revision "%(rev)s" from Gerrit: the response could ' "not be decoded: %(error)s") % {"file": path, "rev": revision, "error": e})
	def get_change(self, repository, revision):
		...
		url = self._build_project_api_url(repository, ("commits", revision, "diff"))
		try:
			diff = self.client.api_get(url, json=False)
		except HostingServiceError as e:
			logger.exception('Could not retrieve change "%s": %s', revision, e)
			raise RepositoryError(ugettext('Could not retrieve change "%(rev)s" from repository ' '%(repo)d: %(error)s"') % {"rev": revision, "repo": repository.id, "error": e})
		commit.diff = diff
		return commit
	def _parse_commit(self, meta, diff=None):
        ...
        Returns:
            ...
            The parsed commit.
        """
		commit = Commit(author_name=meta["author"], id=meta["revision"], date=meta["time"], message=meta["message"], diff=diff)
		if meta["parents"]:
	...
	def _build_project_api_url(self, repository, rest_parts, query=None):
        ...
        Returns:
            ...
            The full URL.
        """
		parts = ["a", "projects", quote_plus(repository.extra_data["gerrit_project_name"])]
