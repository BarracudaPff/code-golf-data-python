def validate_repositories(form, field="repositories"):
	...
	for repository in repositories:
		if repository.local_site != local_site:
			raise ValidationError([_("The repository '%s' doesn't exist on the local site.") % repository.name])
	return repositories
def validate_review_groups(form, field="review_groups"):
	...
	for group in groups:
		if group.local_site != local_site:
			raise ValidationError([_("The review group %s does not exist.") % group.name])
	return groups
	...
	if local_site:
		for user in users:
			if not user.local_site.filter(pk=local_site.pk).exists():
				raise ValidationError([_("The user %s is not a member of this site.") % user.username])
	return users