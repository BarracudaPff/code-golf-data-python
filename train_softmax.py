...
class Trainer:
	...
	def build(self):
		...
		self.train_summary = tf.summary.merge([tf.summary.scalar("inference_loss", self.inference_loss), tf.summary.scalar("wd_loss", self.wd_loss), tf.summary.scalar("train_loss", self.train_loss), tf.summary.scalar("train_acc", self.train_acc)])
