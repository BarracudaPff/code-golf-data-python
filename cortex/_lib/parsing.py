...
_protected_args = ["arch", "out_path", "name", "reload", "args", "copy_to_local", "meta", "config_file", "clean", "verbosity", "test"]
...
def _parse_defaults(key, args, subparser):
	for k, v in args.items():
		...
		if isinstance(v, dict):
			dstr = ",,".join(["{}={}".format(k, str(v)) for k, v in v.items()])
			dstr = dstr.replace(" ", "")
			dstr = dstr.replace("]", "")
			dstr = dstr.replace("[", "")
			metavar = "<k1=v1>" + " defaults={" + dstr + "})"
			subparser.add_argument(arg_str, dest=dest, default=None, action=StoreDictKeyPair, help=help, metavar=metavar)
			...
			metavar = "<" + type_.__name__ + "> (default=" + str(v) + ")"
			subparser.add_argument(arg_str, dest=dest, default=v, metavar=metavar, type=type_, help=help)
def _parse_kwargs(k, v, help, subparser):
	...
	if isinstance(v, dict):
		...
		dstr = dstr.replace("[", "")
		metavar = "<k1=v1>" + " defaults={" + dstr + "})"
		subparser.add_argument(arg_str, dest=k, default=v, action=StoreDictKeyPair, help=help, metavar=metavar)
	elif isinstance(v, bool) and not v:
		action = "store_true"
		...
		type_ = type(v) if v is not None else str
		metavar = "<" + type_.__name__ + "> (default=" + str(v) + ")"
		subparser.add_argument(arg_str, dest=k, choices=choices, metavar=metavar, default=v, type=type_, help=help)
def parse_args(models, model=None):
	...
	if model is None:
		subparsers = parser.add_subparsers(title="Cortex", help="Select an architecture.", description="Cortex is a wrapper " "around pytorch that makes training models " "more convenient.", dest="command")
		subparsers.add_parser("setup", help="Setup cortex configuration.", description="Initializes or updates the `.cortex.yml` file.")
		for k, model in models.items():
			model_help, model_description = parse_header(model)
			subparser = subparsers.add_parser(k, help=model_help, description=model_description, formatter_class=lambda prog: argparse.HelpFormatter(prog, max_help_position=50, width=100))
