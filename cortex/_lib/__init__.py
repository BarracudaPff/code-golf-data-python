...
def setup_experiment(args, model=None, testmode=False):
	...
	reload_nets = None
	if args.autoreload:
		reload_path = find_autoreload(args.out_path, config.CONFIG.out_path, args.name or model_name)
	elif args.reload:
		...
		if args.load_networks:
			...
			for key in keys:
				if key not in d["nets"]:
					raise KeyError("Model {} has no network called {}".format(args.load_networks, key))
