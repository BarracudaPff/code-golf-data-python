...
class TorrentAddingDialog(QDialog):
	...
	def __init__(self, parent: QWidget, filename: str, torrent_info: TorrentInfo, control_thread: "ControlManagerThread"):
		...
		self._selection_label = QLabel(TorrentAddingDialog.SELECTION_LABEL_FORMAT.format(len(download_info.files), humanize_size(download_info.total_size)))
	...
	def _update_selection_label(self):
		...
		if not selected_file_count:
			...
			self._selection_label.setText(TorrentAddingDialog.SELECTION_LABEL_FORMAT.format(selected_file_count, humanize_size(selected_size)))
...
class TorrentListWidgetItem(QWidget):
	...
	def _update(self):
		...
		if self.waiting_control_action:
			...
			status_text = "Downloading from {} of {} peers".format(state.downloading_peer_count, state.total_peer_count)
...
class MainWindow(QMainWindow):
	...
	def _add_torrents_triggered(self):
		paths, _ = QFileDialog.getOpenFileNames(self, "Add torrents", self._control_thread.control.last_torrent_dir, "Torrent file (*.torrent);;All files (*)")
	...
	def _control_action_triggered(self, action):
		for item in self._list_widget.selectedItems():
			...
			asyncio.run_coroutine_threadsafe(MainWindow._invoke_control_action(action, info_hash), self._control_thread.loop)
			widget.waiting_control_action = True
		self._update_control_action_state()
	def _show_about(self):
		QMessageBox.about(self, "About", "<p><b>Prototype of a BitTorrent client</b></p>" "<p>Copyright &copy; 2016 Alexander Borzunov</p>" "<p>Icons are made by Google and Freepik from " '<a href="http://www.flaticon.com">www.flaticon.com</a></p>')
class ControlManagerThread(QThread):
	...
	def stop(self):
		if self._stopping:
			return
		self._stopping = True
		stop_fut = asyncio.run_coroutine_threadsafe(asyncio.wait([self._control_server.stop(), self._control.stop()]), self._loop)
