...
def get_test_function_names_from_class(test_class):
	...
	return {member for member in member_dict if isfunction(member_dict[member]) and member[:5] == "test_"}
...
class TestingInvariants(unittest.TestCase):
	def setUp(self):
		...
		self.expected_test_functions = {"test_" + input_name for input_name in input_names if input_name not in IGNORED_FUNCTIONS}
	def test_ir_generation_test_invariants(self):
		...
		for expected_test_function_name in self.expected_test_functions:
			if expected_test_function_name not in ir_generation_test_names:
				raise AssertionError(u'Test case "{}" not found in test_ir_generation.py.'.format(expected_test_function_name))
	def test_compiler_test_invariants(self):
		...
		for expected_test_function_name in self.expected_test_functions:
			if expected_test_function_name not in compiler_test_names:
				raise AssertionError(u'Test case "{}" not found in test_compiler.py.'.format(expected_test_function_name))