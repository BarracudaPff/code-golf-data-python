...
DirectEdgeDescriptor = namedtuple("DirectEdgeDescriptor", ("from_vertex", "from_column", "to_vertex", "to_column"))
def get_join_descriptors_from_edge_descriptors(direct_edges):
	...
	for edge_name, direct_edge_descriptor in direct_edges.items():
		...
		join_descriptors[direct_edge_descriptor.from_vertex][out_edge_name] = DirectJoinDescriptor(from_column, to_column)
		join_descriptors[direct_edge_descriptor.to_vertex][in_edge_name] = DirectJoinDescriptor(to_column, from_column)
...
def validate_edge_descriptors(vertex_name_to_table, direct_edges):
	...
	for edge_name, direct_edge_descriptor in six.iteritems(direct_edges):
		for vertex_name, column_name in ((direct_edge_descriptor.from_vertex, direct_edge_descriptor.from_column), (direct_edge_descriptor.to_vertex, direct_edge_descriptor.to_column)):
			if vertex_name not in vertex_name_to_table:
				raise InvalidSQLEdgeError("SQL edge {} with edge descriptor {} references a " "non-existent vertex {}".format(edge_name, direct_edge_descriptor, vertex_name))
			if column_name not in vertex_name_to_table[vertex_name].columns:
				raise InvalidSQLEdgeError("SQL edge {} with edge descriptor {} references a " "non-existent column {}".format(edge_name, direct_edge_descriptor, column_name))