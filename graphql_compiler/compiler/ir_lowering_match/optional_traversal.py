...
from ..expressions import BinaryComposition, ContextField, FoldedContextField, Literal, LocalField, OutputContextField, TernaryConditional, TrueLiteral, UnaryTransformation, Variable
from ..match_query import MatchQuery, MatchStep
from .utils import BetweenClause, CompoundMatchQuery, construct_optional_traversal_tree, expression_list_to_conjunction, filter_edge_field_non_existence
def _prune_traverse_using_omitted_locations(match_traversal, omitted_locations, complex_optional_roots, location_to_optional_roots):
	...
	for step in match_traversal:
		...
		if isinstance(step.root_block, Traverse) and step.root_block.optional:
			...
			if optional_root_location is None:
				raise AssertionError(u"Found optional Traverse location {} that was not present " u"in location_to_optional_roots dict: {}".format(current_location, location_to_optional_roots))
...
def _get_present_locations(match_traversals):
	...
	if not present_non_optional_locations.issubset(present_locations):
		raise AssertionError(u"present_non_optional_locations {} was not a subset of " u"present_locations {}. THis hould never happen.".format(present_non_optional_locations, present_locations))
...
def prune_non_existent_outputs(compound_match_query):
	...
	if len(compound_match_query.match_queries) == 1:
		...
		raise AssertionError(u"Received CompoundMatchQuery with " u"an empty list of MatchQuery objects.")
	else:
		match_queries = []
			...
			for output_name, expression in six.iteritems(output_block.fields):
				if isinstance(expression, OutputContextField):
					...
					if location_name not in present_locations:
						raise AssertionError(u"Non-optional output location {} was not found in " u"present_locations: {}".format(expression.location, present_locations))
					...
					if location_name not in present_locations:
						raise AssertionError(u"Folded output location {} was found in " u"present_locations: {}".format(base_location, present_locations))
					new_output_fields[output_name] = expression
					...
					raise AssertionError(u"Invalid expression of type {} in output block: " u"{}".format(type(expression).__name__, output_block))
...
def _apply_filters_to_first_location_occurrence(match_traversal, location_to_filters, already_filtered_locations):
	...
	for match_step in match_traversal:
		...
		if current_location in newly_filtered_locations:
			raise AssertionError(u"The same location {} was encountered twice in a single " u"match traversal: {}. This should never happen.".format(current_location, match_traversal))
...
def _update_context_field_binary_composition(present_locations, expression):
	...
	if not any((isinstance(expression.left, ContextField), isinstance(expression.right, ContextField))):
		raise AssertionError(u"Received a BinaryComposition {} without any ContextField " u"operands. This should never happen.".format(expression))
...
def _simplify_non_context_field_binary_composition(expression):
	...
	if any((isinstance(expression.left, ContextField), isinstance(expression.right, ContextField))):
		raise AssertionError(u"Received a BinaryComposition {} with a ContextField " u"operand. This should never happen.".format(expression))
...
def _update_context_field_expression(present_locations, expression):
	...
	if isinstance(expression, BinaryComposition):
		...
		if isinstance(lower_bound, ContextField) or isinstance(upper_bound, ContextField):
			raise AssertionError(u"Found BetweenClause with ContextFields as lower/upper bounds. " u"This should never happen: {}".format(expression))
		return expression
	elif isinstance(expression, (OutputContextField, FoldedContextField)):
		raise AssertionError(u"Found unexpected expression of type {}. This should never happen: " u"{}".format(type(expression).__name__, expression))
	elif isinstance(expression, no_op_blocks):
		return expression
	raise AssertionError(u"Found unhandled expression of type {}. This should never happen: " u"{}".format(type(expression).__name__, expression))
...
def lower_context_field_expressions(compound_match_query):
	...
	if len(compound_match_query.match_queries) == 0:
		raise AssertionError(u"Received CompoundMatchQuery {} with no MatchQuery objects.".format(compound_match_query))
		...
		for match_query in compound_match_query.match_queries:
			...
			new_match_traversals = _lower_non_existent_context_field_filters(match_traversals, current_visitor_fn)
			new_match_queries.append(MatchQuery(match_traversals=new_match_traversals, folds=match_query.folds, output_block=match_query.output_block, where_block=match_query.where_block))
	return CompoundMatchQuery(match_queries=new_match_queries)