...
class TaskController:
	def __init__(self, tasks_dir="etc", history_dir="./var", notifier_config={}, notify_to="logger", notifier_host="hostname", websocket_clients=[], port=8888, datadog_config={}):
		self.tasks_dir = tasks_dir
		if not os.path.exists(self.tasks_dir):
			os.makedirs(self.tasks_dir)
		self.websocket_clients = websocket_clients
		self.history_storage = FusionHistoryStorage(history_dir)
		self.notifier = Notifier(notifier_config=notifier_config, notify_to=notify_to, websocket_clients=websocket_clients, host=notifier_host, port=port)
		self.task_scheduler = TasksScheduler(tasks_dir=tasks_dir, history_storage=self.history_storage, notifier=self.notifier, datadog_config=datadog_config)
	...
	def run_task_by_task_id(self, task_id):
		...
		self.task_scheduler.run_task_by_name_and_cmd(task["name"], task["cmd"], task.get("timeout", -1), task.get("fails_before_notify", 0))
