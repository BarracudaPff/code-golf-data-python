...
class SecretKey(ndb.Model):
    ...
    """
		...
		config = cls(id=cls.GLOBAL_KEY, client_id=client_id, client_secret=client_secret, additional_client_ids=additional_client_ids)
		config.put()
		return config
	@classmethod
	def get_config(cls):
    ...
    """
		...
		if config is None:
			...
			return (config.client_id, config.client_secret, config.additional_client_ids)
...
def get_current_rietveld_oauth_user():
  ...
  """
	...
	if accepted_client_id != current_client_id and current_client_id not in additional_client_ids:
		logging.debug("Client ID %r not intended for this application.", current_client_id)
		return
	try:
...
class AnyAuthUserProperty(ndb.UserProperty):
  ...
  """
    ...
    """
		if self._auto_current_user or (self._auto_current_user_add and not self._has_value(entity)):
