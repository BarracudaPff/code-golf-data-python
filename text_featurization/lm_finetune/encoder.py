...
class encoder:
	...
	def embed(self, x):
		...
		if model_name == "elmo":
			...
			executable = elmo(tf.squeeze(tf.cast(x, tf.string)), signature="default", as_dict=True)["default"]
		elif model_name == "nnlm":
			nnlm = hub.Module(NNLM)
			executable = nnlm(tf.squeeze(tf.cast(x, tf.string)))
		elif model_name == "use":
			encoder = hub.Module(USE)
			executable = encoder(tf.squeeze(tf.cast(x, tf.string)), signature="default", as_dict=True)["default"]
		else:
	...
	def transform_model(self):
		...
		embedding = layers.Lambda(self.embed, output_shape=(embed_size,))(input_text)
...
if __name__ == "__main__":
	...
	with tf.Session() as session:
		...
		session.run(tf.global_variables_initializer())
		session.run(tf.tables_initializer())
		history = trainable_model.fit(imdb_df.review.values[:25000], y_train, batch_size=8)
	...
	g_train_sizes, g_train_scores, g_test_scores = learning_curve(estimator=estimator, X=glove_features, y=labels, scoring=make_scorer(roc_auc_score), n_jobs=8, train_sizes=train_sizes)
	e_train_sizes, e_train_scores, e_test_scores = learning_curve(estimator=estimator, X=use_features, y=labels, scoring=make_scorer(roc_auc_score), n_jobs=8, train_sizes=train_sizes)
	results_df = pd.DataFrame({"train_perc": train_sizes, "bow_auc": np.mean(g_test_scores, axis=1), "encoder_auc": np.mean(e_test_scores, axis=1)})
	sns.lineplot(x="train_perc", y="AUC", hue="features", data=results_df.melt("train_perc", var_name="features", value_name="AUC"))
	...
	def cv_predict_eval():
		cv = GridSearchCV(RandomForestClassifier(), param_grid={"n_estimators": [10, 100], "max_features": ["sqrt", "log2"], "max_depth": [3, 5, None]}, refit=True, n_jobs=20)
