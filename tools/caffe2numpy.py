...
def convert_weights(layers, v="V1"):
	...
	for name, layer in layers.items():
		...
		if typ == "innerproduct":
			...
			weights[name] = [np.ones(nb_kernels), np.zeros(nb_kernels), weights_mean.astype(dtype=np.float32), weights_std_dev.astype(dtype=np.float32)]
			print(" > {} {}: weights_mean={}, weights_std_dev={}".format(typ, name, weights_mean.shape, weights_std_dev.shape))
		elif typ == "Scale":
			blobs = layer.blobs
			weights[name] = [np.array(blobs[0].data, dtype=np.float32), np.array(blobs[1].data, dtype=np.float32)]
	...
	return weights
def load_caffe(path_prototxt="weights/resnetFCN.prototxt", path_weights="weights/resnetFCN.caffemodel", out_path="weights/resnetFCN.npy", version="V1"):
...
if __name__ == "__main__":
	...
	parser.add_argument("-p", "--path_prototxt", type=str, default="weights/resnetFCN.prototxt", help="Path to the proto txt caffe file")
	parser.add_argument("-w", "--path_weights", type=str, default="weights/resnetFCN.caffemodel", help="Path to the caffe weights file")
	parser.add_argument("-o", "--out_path", type=str, default="weights/resnetFCN.npy", help="Output path")
	parser.add_argument("-v", "--version", type=int, default=1, help="Caffe model version (1 or 2)")
