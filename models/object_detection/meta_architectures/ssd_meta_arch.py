...
class SSDMetaArch(model.DetectionModel):
    ...
    """
		...
		self._anchors = self._anchor_generator.generate(feature_map_spatial_dims, im_height=image_shape[1], im_width=image_shape[2])
		(box_encodings, class_predictions_with_background) = self._add_box_predictions_to_feature_maps(feature_maps)
		predictions_dict = {"box_encodings": box_encodings, "class_predictions_with_background": class_predictions_with_background, "feature_maps": feature_maps, "anchors": self._anchors.get()}
    ...
    """
		...
		with tf.name_scope("Postprocessor"):
			...
			(nmsed_boxes, nmsed_scores, nmsed_classes, _, nmsed_additional_fields, num_detections) = self._non_max_suppression_fn(detection_boxes, detection_scores, clip_window=clip_window, additional_fields=additional_fields)
			detection_dict = {"detection_boxes": nmsed_boxes, "detection_scores": nmsed_scores, "detection_classes": nmsed_classes, "num_detections": tf.to_float(num_detections)}
			if nmsed_additional_fields is not None and fields.BoxListFields.keypoints in nmsed_additional_fields:
				detection_dict["detection_keypoints"] = nmsed_additional_fields[fields.BoxListFields.keypoints]
    ...
    """
		with tf.name_scope(scope, "Loss", prediction_dict.values()):
			...
			(batch_cls_targets, batch_cls_weights, batch_reg_targets, batch_reg_weights, match_list) = self._assign_targets(self.groundtruth_lists(fields.BoxListFields.boxes), self.groundtruth_lists(fields.BoxListFields.classes), keypoints)
			...
			with tf.name_scope("localization_loss"):
				localization_loss = (self._localization_loss_weight / normalizer) * localization_loss
	...
	def _assign_targets(self, groundtruth_boxes_list, groundtruth_classes_list, groundtruth_keypoints_list=None):
    ...
    Returns:
      ...
      mined_cls_loss: a float scalar with sum of classification losses from
        selected hard examples.
    """
		class_predictions = tf.slice(prediction_dict["class_predictions_with_background"], [0, 0, 1], [-1, -1, -1])
