...
class MaskRCNNBoxPredictorTest(tf.test.TestCase):
	...
	def test_get_boxes_with_five_classes(self):
		...
		with self.test_session() as sess:
			sess.run(init_op)
			(box_encodings_shape, class_predictions_with_background_shape) = sess.run([tf.shape(box_encodings), tf.shape(class_predictions_with_background)])
...
class RfcnBoxPredictorTest(tf.test.TestCase):
	...
	def test_get_correct_box_encoding_and_class_prediction_shapes(self):
		...
		rfcn_box_predictor = box_predictor.RfcnBoxPredictor(is_training=False, num_classes=2, conv_hyperparams=self._build_arg_scope_with_conv_hyperparams(), num_spatial_bins=[3, 3], depth=4, crop_size=[12, 12], box_code_size=4)
		...
		with self.test_session() as sess:
			sess.run(init_op)
			(box_encodings_shape, class_predictions_shape) = sess.run([tf.shape(box_encodings), tf.shape(class_predictions_with_background)])
...
class ConvolutionalBoxPredictorTest(tf.test.TestCase):
	...
	def test_get_boxes_for_five_aspect_ratios_per_location(self):
		...
		with self.test_session() as sess:
			sess.run(init_op)
			(box_encodings_shape, objectness_predictions_shape) = sess.run([tf.shape(box_encodings), tf.shape(objectness_predictions)])
	...
	def test_get_multi_class_predictions_for_five_aspect_ratios_per_location(self):
		...
		with self.test_session() as sess:
			...
			(box_encodings_shape, class_predictions_with_background_shape) = sess.run([tf.shape(box_encodings), tf.shape(class_predictions_with_background)])
			self.assertAllEqual(box_encodings_shape, [4, 320, 1, 4])
			self.assertAllEqual(class_predictions_with_background_shape, [4, 320, num_classes_without_background + 1])
