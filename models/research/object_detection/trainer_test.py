...
def get_input_function():
	...
	class_label = tf.random_uniform([1], minval=0, maxval=NUMBER_OF_CLASSES, dtype=tf.int32)
	box_label = tf.random_uniform([1, 4], minval=0.4, maxval=0.6, dtype=tf.float32)
	return {fields.InputDataFields.image: image, fields.InputDataFields.key: key, fields.InputDataFields.groundtruth_classes: class_label, fields.InputDataFields.groundtruth_boxes: box_label}
class FakeDetectionModel(model.DetectionModel):
    ...
    """
		...
		return {"class_predictions_with_background": tf.reshape(class_prediction, [-1, 1, self._num_classes]), "box_encodings": tf.reshape(box_prediction, [-1, 1, 4])}
    ...
    """
		...
		loss_dict = {"localization_loss": tf.reduce_sum(location_losses), "classification_loss": tf.reduce_sum(cls_losses)}
		return loss_dict
