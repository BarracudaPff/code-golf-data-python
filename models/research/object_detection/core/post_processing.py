...
def multiclass_non_max_suppression(boxes, scores, score_thresh, iou_thresh, max_size_per_class, max_total_size=0, clip_window=None, change_coordinate_frame=False, masks=None, additional_fields=None, scope=None):
  ...
  """
	...
	if change_coordinate_frame and clip_window is None:
		raise ValueError("if change_coordinate_frame is True, then a clip_window" "must be specified.")
	with tf.name_scope(scope, "MultiClassNonMaxSuppression"):
		num_boxes = tf.shape(boxes)[0]
		num_scores = tf.shape(scores)[0]
		num_classes = scores.get_shape()[1]
		length_assert = tf.Assert(tf.equal(num_boxes, num_scores), ["Incorrect scores field length: actual vs expected.", num_scores, num_boxes])
		selected_boxes_list = []
		...
		return sorted_boxes
def batch_multiclass_non_max_suppression(boxes, scores, score_thresh, iou_thresh, max_size_per_class, max_total_size=0, clip_window=None, change_coordinate_frame=False, num_valid_boxes=None, masks=None, additional_fields=None, scope=None, parallel_iterations=32):
  ...
  """
	...
	with tf.name_scope(scope, "BatchMultiClassNonMaxSuppression"):
      ...
      """
			...
			per_image_additional_fields = {key: value for key, value in zip(additional_fields, args[3:-1])}
			...
			nmsed_boxlist = multiclass_non_max_suppression(per_image_boxes, per_image_scores, score_thresh, iou_thresh, max_size_per_class, max_total_size, clip_window=clip_window, change_coordinate_frame=change_coordinate_frame, masks=per_image_masks, additional_fields=per_image_additional_fields)
			...
			nmsed_additional_fields = [padded_boxlist.get_field(key) for key in per_image_additional_fields]
		...
		batch_outputs = tf.map_fn(_single_image_nms_fn, elems=([boxes, scores, masks] + list(additional_fields.values()) + [num_valid_boxes]), dtype=(num_nmsed_outputs * [tf.float32] + [tf.int32]), parallel_iterations=parallel_iterations)
