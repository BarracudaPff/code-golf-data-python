...
flags.DEFINE_string("task_context", "", "Path to a task context with resource locations and " "parameters.")
...
flags.DEFINE_string("word_embeddings", None, "Recordio containing pretrained word embeddings, will be " "loaded as the first embedding matrix.")
...
flags.DEFINE_string("graph_builder", "greedy", 'Graph builder to use, either "greedy" or "structured".')
...
flags.DEFINE_bool("slim_model", False, "Whether to remove non-averaged variables, for compactness.")
flags.DEFINE_float("learning_rate", 0.1, "Initial learning rate parameter.")
flags.DEFINE_integer("decay_steps", 4000, "Decay learning rate by 0.96 every this many steps.")
...
flags.DEFINE_string("pretrained_params", None, "Path to model from which to load params.")
flags.DEFINE_string("pretrained_params_names", None, "List of names of tensors to load from pretrained model.")
flags.DEFINE_float("averaging_decay", 0.9999, "Decay for exponential moving average when computing" "averaged parameters, set to 1 to do vanilla averaging.")
def StageName():
