...
MAPPING = {"object": "String", "uint64": "UInt64", "uint32": "UInt32", "uint16": "UInt16", "uint8": "UInt8", "float64": "Float64", "float32": "Float32", "int64": "Int64", "int32": "Int32", "int16": "Int16", "int8": "Int8", "datetime64[D]": "Date", "datetime64[ns]": "DateTime"}
PD2CH = keymap(np.dtype, MAPPING)
CH2PD = itemmap(reversed, MAPPING)
CH2PD["Null"] = "object"
CH2PD["Nothing"] = "object"
NULLABLE_COLS = ["UInt64", "UInt32", "UInt16", "UInt8", "Float64", "Float32", "Int64", "Int32", "Int16", "Int8", "String"]
...
def normalize(df, index=True):
	...
	return dtypes, df
def to_csv(df):
	data = df.to_csv(header=False, index=False, encoding="utf-8", quoting=csv.QUOTE_NONNUMERIC, escapechar="\\")
	if PY3:
...
def to_dataframe(lines, **kwargs):
	...
	for name, chtype in zip(names, types):
		...
		if chtype.startswith("Array("):
			...
			dtypes[name] = dtype
	return pd.read_table(lines, header=None, names=names, dtype=dtypes, parse_dates=parse_dates, converters=converters, na_values=set(), keep_default_na=False, **kwargs)
