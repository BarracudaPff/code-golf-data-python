...
class Vqe:
	def __init__(self, ansatz, minimizer=None, sampler=None):
		self.ansatz = ansatz
		self.minimizer = minimizer or get_scipy_minimizer(method="Powell", options={"ftol": 5.0e-2, "xtol": 5.0e-2, "maxiter": 1000})
	...
	def result(self):
		...
		warnings.warn("Vqe.result is deprecated. Use `result = Vqe.run()`", DeprecationWarning)
