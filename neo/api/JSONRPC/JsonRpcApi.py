...
class JsonRpcApi:
	def __init__(self, wallet=None):
		...
		cors = aiohttp_cors.setup(self.app, defaults={"*": aiohttp_cors.ResourceOptions(allow_headers=("Content-Type", "Access-Control-Allow-Headers", "Authorization", "X-Requested-With"))})
	...
	def options_response(cls):
		return {"supported HTTP methods": ("GET", "POST"), "JSON-RPC server type": "default"}
	def json_rpc_method_handler(self, method, params):
		if method == "getaccountstate":
			...
			return {"port": self.port, "nonce": self.nodemgr.id, "useragent": settings.VERSION_NAME}
	...
	def get_invoke_result(self, script):
		...
		return {"script": script.decode("utf-8"), "state": VMStateStr(appengine.State), "gas_consumed": appengine.GasConsumed().ToString(), "stack": [ContractParameter.ToParameter(item).ToJson() for item in appengine.ResultStack.Items]}
	...
	def list_address(self):
		...
		for addrStr in self.wallet.Addresses:
			...
			result.append({"address": addrStr, "haskey": not addr.IsWatchOnly, "label": None, "watchonly": addr.IsWatchOnly})
		return result
	def parse_send_to_address_params(self, params):
		...
		if not amount or float(params[2]) == 0:
			raise JsonRpcError(-32602, "Invalid params")
		output = TransactionOutput(AssetId=asset_id, Value=amount, script_hash=address_to_sh)
	...
	def parse_send_many_params(self, params):
		...
		for info in params[0]:
			...
			if not amount or float(info["value"]) == 0:
				raise JsonRpcError(-32602, "Invalid params")
			tx_output = TransactionOutput(AssetId=asset, Value=amount, script_hash=address)
	...
	def process_transaction(self, contract_tx, fee=None, address_from=None, change_addr=None):
		...
		try:
			tx = self.wallet.MakeTransaction(tx=contract_tx, change_address=change_addr, fee=fee, from_addr=address_from)
		except ValueError:
			raise JsonRpcError(-300, "Insufficient funds")
		if tx is None:
			raise JsonRpcError(-300, "Insufficient funds")
		data = standard_contract.Data
		tx.Attributes = [TransactionAttribute(usage=TransactionAttributeUsage.Script, data=data)]
