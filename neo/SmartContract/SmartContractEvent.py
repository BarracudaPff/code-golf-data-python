...
class SmartContractEvent(SerializableMixin):
	...
	def __str__(self):
		return "SmartContractEvent(event_type=%s, event_payload=%s, contract_hash=%s, block_number=%s, tx_hash=%s, execution_success=%s, test_mode=%s)" % (self.event_type, self.event_payload, self.contract_hash, self.block_number, self.tx_hash, self.execution_success, self.test_mode)
	...
	def ToJson(self):
		jsn = {"type": self.event_type, "contract": self.contract_hash.To0xString(), "block": self.block_number, "tx": self.tx_hash.To0xString()}
		if self.event_type in [SmartContractEvent.CONTRACT_CREATED, SmartContractEvent.CONTRACT_MIGRATED]:
	...
	def __init__(self, event_type, event_payload, contract_hash, block_number, tx_hash, execution_success=False, test_mode=False):
		...
		if self.event_payload.Type == ContractParameterType.Array and len(self.event_payload.Value) > 0:
			...
			try:
				...
				elif self.notify_type == NotifyType.REFUND and plen >= 3:
