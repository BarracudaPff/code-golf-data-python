...
def getip():
	...
	return str(ip)
def open_port(port):
	cmd = ["iptables -I INPUT -m state --state NEW -m tcp -p tcp --dport $1 -j ACCEPT", "iptables -I INPUT -m state --state NEW -m udp -p udp --dport $1 -j ACCEPT", "ip6tables -I INPUT -m state --state NEW -m tcp -p tcp --dport $1 -j ACCEPT", "ip6tables -I INPUT -m state --state NEW -m udp -p udp --dport $1 -j ACCEPT"]
	for x in cmd:
	...
	server_raw = """
{
	...
	if data["protocol"] == "vmess":
		...
		server["routing"]["settings"]["rules"].append({"type": "field", "inboundTag": ["tg-in"], "outboundTag": "tg-out"})
	if data["trans"] == "tcp":
