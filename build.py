...
version = tuple(map(int, (re.search(r"^#define MAJOR (\d+)", raw, flags=re.MULTILINE).group(1), re.search(r"^#define MINOR (\d+)", raw, flags=re.MULTILINE).group(1), re.search(r"^#define PATCH (\d+)", raw, flags=re.MULTILINE).group(1))))
...
def cc_version():
	...
	ver = int(re.search(br"#define __GNUC__ (\d+)", raw, flags=re.M).group(1)), int(re.search(br"#define __GNUC_MINOR__ (\d+)", raw, flags=re.M).group(1))
...
def init_env(debug=False, sanitize=False, native_optimizations=False, add_python=True):
	...
	cflags = os.environ.get("OVERRIDE_CFLAGS", ("-Wextra -Wno-missing-field-initializers -Wall -std=c99 -fvisibility=hidden" " -pedantic-errors -Werror {} {} -D{}DEBUG -fwrapv {} {} -pipe {}").format(optimize, " ".join(sanitize_args), ("" if debug else "N"), stack_protector, missing_braces, "-march=native" if native_optimizations else ""))
	...
	ldflags = os.environ.get("OVERRIDE_LDFLAGS", "-Wall -shared " + " ".join(sanitize_args) + ("" if debug else " -O3"))
...
def build(args, build_leak_check=False):
	...
	if build_leak_check and newer(MEMLEAK_EXE, "mem-leak-check.c", *debug_objects):
		cmd = [debug_env.cc] + debug_env.cflags + ["mem-leak-check.c"] + ["-o", MEMLEAK_EXE] + debug_objects + debug_env.ldflags
...
def option_parser():
	...
	p.add_argument("action", nargs="?", default="test", choices="build test try leak".split(), help="Action to perform (default is build)")
...
def main():
	...
	if args.action == "build":
		...
		os.environ["ASAN_OPTIONS"] = "leak_check_at_exit=0"
		add_python_path(os.environ, os.path.dirname(freeze_dir))
		os.execlp(TEST_EXE, TEST_EXE, "-c", "from html5_parser import *; " + args.rest[0], *args.rest[1:])
