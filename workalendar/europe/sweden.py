...
class Sweden(WesternCalendar, ChristianMixin):
	...
	FIXED_HOLIDAYS = WesternCalendar.FIXED_HOLIDAYS + ((5, 1, "Labour Day"), (6, 6, "National Day"), (12, 31, "New Year's Eve"))
	def get_midsummer_eve(self, year):
		date_eve = Sweden.get_nth_weekday_in_month(year, 6, FRI, start=date(year, 6, 19))
		return date_eve
	def get_midsummer_day(self, year):
		date_eve = Sweden.get_nth_weekday_in_month(year, 6, SAT, start=date(year, 6, 20))
		return date_eve
