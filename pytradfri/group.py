...
from .const import ROOT_GROUPS, ATTR_DEVICE_STATE, ATTR_LIGHT_DIMMER, ATTR_LIGHT_COLOR_X, ATTR_LIGHT_COLOR_Y, ATTR_LIGHT_COLOR_HEX, ATTR_ID, ATTR_GROUP_MEMBERS, ATTR_MOOD, ATTR_HS_LINK, ATTR_TRANSITION_TIME, RANGE_X, RANGE_Y, RANGE_MIREDS, ATTR_LIGHT_MIREDS, RANGE_HUE, RANGE_SATURATION, ATTR_LIGHT_COLOR_SATURATION, ATTR_LIGHT_COLOR_HUE, RANGE_BRIGHTNESS
...
class Group(ApiResource):
	...
	def activate_mood(self, mood_id):
		"""Activate a mood."""
		return self.set_values({ATTR_MOOD: mood_id})
	...
	def _value_validate(self, value, rnge, identifier="Given"):
		...
		if value is not None and (value < rnge[0] or value > rnge[1]):
			raise ValueError("%s value must be between %d and %d." % (identifier, rnge[0], rnge[1]))
	def __repr__(self):
