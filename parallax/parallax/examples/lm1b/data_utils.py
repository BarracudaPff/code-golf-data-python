...
class Dataset(object):
	...
	def iterate_once(self, batch_size, num_steps):
		...
		for value in self._iterate(self._sentence_stream(file_stream()), batch_size, num_steps):
			yield value
	def iterate_forever(self, batch_size, num_steps, num_workers, worker_id):
		def file_stream():
			while True:
				file_patterns = glob.glob(self._file_pattern)
				file_patterns.sort()
				filenames_for_worker = []
				for i in range(len(file_patterns)):
					if i % num_workers == worker_id:
						filenames_for_worker.append(file_patterns[i])
				if not self._deterministic:
					random.shuffle(filenames_for_worker)
				for filename in filenames_for_worker:
					yield filename
		for value in self._iterate(self._sentence_stream(file_stream()), batch_size, num_steps):
			yield value