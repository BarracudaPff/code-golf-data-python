...
def _add_broadcast_ops(target, worker_id):
	...
	with tf.device("/job:worker/task:%d" % worker_id):
		for var in target:
			bcast_global_variables_ops.append(tf.assign(var, hvd.broadcast(var, 0)))
...
def graph_transform_hybrid(single_gpu_meta_graph_def, worker_id, local_worker_id, machine_id, hostname, config):
	...
	multi_gpu_meta_graph_def = in_graph_auto_parallel_compute(single_gpu_meta_graph_def, worker_id, local_worker_id, num_local_workers, config, None, tensor_or_op_name_to_replica_names)
	local_aggregation = config.communication_config.ps_config.local_aggregation
	ps_meta_graph_def, var_op_to_agg_grad, trainable_var_op_to_update_op = between_graph_auto_parallel_compute(multi_gpu_meta_graph_def, worker_id=worker_id, local_worker_id=local_worker_id, machine_id=machine_id, num_local_workers=num_local_workers, num_worker_machines=num_worker_machines, ps_device=ps_device, worker_device="/job:worker/task:%d" % worker_id, merge_devices=True, cluster_spec=cluster_spec, sync=True, op_library_path=None, num_replicas_per_worker=1, average_sparse=config.average_sparse, tensor_or_op_name_to_replica_names=tensor_or_op_name_to_replica_names, local_aggregation=local_aggregation)
	meta_graph_def, dense_var_op_to_agg_grad = graph_transform_dense_mpi(worker_id, ps_meta_graph_def, op_library_path=None, config=config)
	with tf.Graph().as_default() as graph:
		tf.train.import_meta_graph(meta_graph_def)
		var_op_to_agg_grad_ = {}
		...
		for var_op_name in dense_var_op_to_agg_grad.keys():
			...
			if var_op in var_op_to_agg_grad_:
				...
				var_op_to_agg_grad_[var_op] = (None, grad)
		if config.communication_config.ps_config.boundary_among_servers:
			_place_post_grad_agg_ops_hybrid(tf.DeviceSpec.from_string(ps_device), var_op_to_agg_grad_, trainable_var_op_to_update_op_)
