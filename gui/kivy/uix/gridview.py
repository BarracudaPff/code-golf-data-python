...
class GridView(BoxLayout):
	...
	def on_headers(self, instance, value):
		...
		if len(self.widths) != len(value):
			return
		def generic_args_converter(row_index, item, is_header=True, getter=self.getter):
			cls_dicts = []
			_widths = self.widths
			getter = self.getter
			on_context_menu = self.on_context_menu
			for i, header in enumerate(self.headers):
				kwargs = {"padding": ("2dp", "2dp"), "halign": "center", "valign": "middle", "size_hint_y": None, "shorten": True, "height": "30dp", "text_size": (_widths[i], dp(30)), "text": getter(item, i)}
				kwargs["font_size"] = "9sp"
				if is_header:
					kwargs["deselected_color"] = kwargs["selected_color"] = [0, 1, 1, 1]
				else:
				...
				cls_dicts.append({"cls": ListItemButton, "kwargs": kwargs})
			return {"id": item[-1], "size_hint_y": None, "height": "30dp", "cls_dicts": cls_dicts}
		...
		self.ids.header_view.adapter = ListAdapter(data=[self.headers], args_converter=header_args_converter, selection_mode="single", allow_empty_selection=False, cls=CompositeListItem)
		self.ids.content_view.adapter = ListAdapter(data=self.data, args_converter=content_args_converter, selection_mode="single", allow_empty_selection=False, cls=CompositeListItem)
