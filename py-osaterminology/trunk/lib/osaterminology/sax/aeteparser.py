...
class _BigEndianDataSource:
	...
	def string(self):
		...
		self._ptr += 1 + count
		if count:
			return unicode(self._str[self._ptr - count : self._ptr], "MacRoman")
		else:
			return u""
	name = string
	def align(self):
...
def _parseProperty(aete, receiver):
	...
	if code == _kAEInheritedProperties:
		...
		receiver.add_property(code, name, description, datatype, bool(flags & 16384), bool(flags & 8192), bool(flags & 4096))
def _parseClass(aete, receiver):
	...
	receiver.end_class()
def _parseComparison(aete, receiver):
	aete.string()
	aete.align()
	aete.word()
	aete.string()
