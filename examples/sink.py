...
ARGS.add_argument("--tls", action="store_true", dest="tls", default=False, help="Use TLS with a self-signed cert")
...
ARGS.add_argument("--port", action="store", dest="port", default=1111, type=int, help="Port number")
ARGS.add_argument("--maxsize", action="store", dest="maxsize", default=16 * 1024 * 1024, type=int, help="Max total data size")
server = None
args = None
def dprint(*args):
...
def start(loop, host, port):
	...
	dprint("serving TLS" if sslctx else "serving", [s.getsockname() for s in server.sockets])
