...
ARGS.add_argument("--tls", action="store_true", dest="tls", default=False, help="Use TLS")
ARGS.add_argument("--iocp", action="store_true", dest="iocp", default=False, help="Use IOCP event loop (Windows only)")
ARGS.add_argument("--host", action="store", dest="host", default="localhost", help="Host name")
ARGS.add_argument("--port", action="store", dest="port", default=54321, type=int, help="Port number")
ARGS.add_argument("--timeout", action="store", dest="timeout", default=5, type=float, help="Timeout")
ARGS.add_argument("--random_failure_percent", action="store", dest="fail_percent", default=0, type=float, help="Fail randomly N percent of the time")
ARGS.add_argument("--random_failure_sleep", action="store", dest="fail_sleep", default=0, type=float, help="Sleep time when randomly failing")
ARGS.add_argument("--random_response_sleep", action="store", dest="resp_sleep", default=0, type=float, help="Sleep time before responding")
...
class Cache:
	...
	def frame_parser(self, reader, writer):
		last_request_id = 0
		while True:
			framing_b = yield from asyncio.wait_for(reader.readline(), timeout=args.timeout, loop=self.loop)
			if random.random() * 100 < args.fail_percent:
				logging.warn("Inserting random failure")
				yield from asyncio.sleep(args.fail_sleep * random.random(), loop=self.loop)
			...
			request_b = yield from asyncio.wait_for(reader.readexactly(byte_count), timeout=args.timeout, loop=self.loop)
			try:
			...
			yield from asyncio.sleep(args.resp_sleep * random.random(), loop=self.loop)
...
def main():
	...
	if args.tls:
		...
		sslctx.load_cert_chain(certfile=os.path.join(here, "ssl_cert.pem"), keyfile=os.path.join(here, "ssl_key.pem"))
	cache = Cache(loop)
	task = asyncio.streams.start_server(cache.handle_client, args.host, args.port, ssl=sslctx, loop=loop)
